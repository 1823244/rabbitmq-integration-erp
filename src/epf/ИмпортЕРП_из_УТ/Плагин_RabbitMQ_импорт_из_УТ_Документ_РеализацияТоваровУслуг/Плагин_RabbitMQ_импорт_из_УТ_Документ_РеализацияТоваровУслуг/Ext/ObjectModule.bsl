...
//	КонецЕсли;
	...
	ОбъектДанных.Соглашение = 
		РегистрыСведений.ксп_ДополнительныеНастройкиИнтеграций.Настройка("СоглашениеСКлиентамиДляРеализацияТоваровУслугИзУТ", мВнешняяСистема);
	...
	//	СтрокаТЧ.СрокПоставки = стрк.СрокПоставки;
		...
		СтрокаТЧ.СтавкаНДС = _Номенклатура.СтавкаНДС;
	...
	//	КонецЕсли;
	//	// на случай, если есть метод поиска ссылки:
	//	СтрокаТЧ.ВалютаВзаиморасчетов = ксп_ИмпортСлужебный.НайтиВалюту(стрк.ВалютаВзаиморасчетов);
