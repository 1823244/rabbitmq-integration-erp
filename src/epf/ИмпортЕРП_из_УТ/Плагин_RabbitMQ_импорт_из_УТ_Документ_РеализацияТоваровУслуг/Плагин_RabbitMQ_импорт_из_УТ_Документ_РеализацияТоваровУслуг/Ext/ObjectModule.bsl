...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	Попытка
		...
		мЛоггер.инфо("Загружен документ "+Строка(ОбъектДанных));
		
	Исключение
        мЛоггер.ерр("Ошибка. Плагин_RabbitMQ_импорт_из_УТ_Документ_РеализацияТоваровУслуг. Подробности: "+ОписаниеОшибки());
		ВызватьИсключение;
	КонецПопытки;
...
//	КонецЕсли;
	...
	ОбъектДанных.Комментарий ="[УТ №"+деф.Number+" от "+строка(деф.Date)+"]" +"Оригинальный коммент.: "+Строка(деф.Комментарий);
	...
	//	СтрокаТЧ.Назначение = ксп_ИмпортСлужебный.НайтиНазначение(стрк.Назначение);
		...
		Если СобиратьНенайденнуюНоменклатуру и ТэгНоменклатуры.Свойство("ref") Тогда
			Если ТипЗнч(_Номенклатура) = Тип("СправочникСсылка.Номенклатура") 
				И НЕ ЗначениеЗаполнено(_Номенклатура.ВерсияДанных) Тогда
...
//	КонецЦикла;
	...
	Попытка
		ксп_ЭкспортСлужебный.ВыполнитьЭкспорт_ГуидовНоменклатуры(НеНайденнаяНоменклатураМассив, "goods.guid" );
	...
	Попытка
		ксп_ЭкспортСлужебный.ВыполнитьЭкспорт_ГуидовКонтрагентов(НеНайденнаяНоменклатураМассив, "goods.guid" );
		Сообщить("Выполнен экспорт ненайденных контрагентов - " + Строка(НеНайденнаяНоменклатураМассив.Количество()) + " позиций");
	Исключение
	    т = ОписаниеОшибки();
		Сообщить("Ошибка экспорта ненайденных контрагентов в УПП.");
        ЗаписьЖурналаРегистрации(ИмяСобытияЖР, УровеньЖурналаРегистрации.Предупреждение,,,
		"Ошибка экспорта ненайденных контрагентов в УПП. Подробности: "+т);
	КонецПопытки;            
