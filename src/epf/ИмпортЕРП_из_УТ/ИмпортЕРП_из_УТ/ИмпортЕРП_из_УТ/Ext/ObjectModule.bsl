...
Процедура ВыполнитьИмпорт() Экспорт
	...
	Исключение
		...
		"Импорт из УТ не выполнен! Причина: Не удалось получить плагины из справочника <Дополнительные отчеты и обработки>. Подробнее: " + т);
		Сообщить( "Импорт из УТ не выполнен! Причина: Не удалось получить плагины из справочника <Дополнительные отчеты и обработки>. Подробнее: " + т );
	КонецПопытки;
	...
	Исключение
	    т = ОписаниеОшибки();
		ЗаписьЖурналаРегистрации(имя_лога,УровеньЖурналаРегистрации.Ошибка,,,
		"Импорт НСИ из УТ не выполнен! Причина: " + т);
		Сообщить( "Импорт НСИ из УТ не выполнен! Причина: " + т );
	КонецПопытки;
	...
	Исключение
	    т = ОписаниеОшибки();
		ЗаписьЖурналаРегистрации(имя_лога,УровеньЖурналаРегистрации.Ошибка,,,
		"Импорт Документов из УТ не выполнен! Причина: " + т);
		Сообщить( "Импорт Документов из УТ не выполнен! Причина: " + т );
	КонецПопытки;
