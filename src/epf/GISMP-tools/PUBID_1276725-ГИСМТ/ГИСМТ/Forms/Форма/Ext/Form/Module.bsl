...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Сервер.СписокВыбора.Добавить("markirovka.sandbox.crptech.ru");
	Элементы.Сервер.СписокВыбора.Добавить("markirovka.crpt.ru");
	Элементы.Сервер.СписокВыбора.Добавить("ismp.crpt.ru");
	Элементы.Сервер.СписокВыбора.Добавить("suz.sandbox.crptech.ru");//СУЗ песочница
	Элементы.Сервер.СписокВыбора.Добавить("suzgrid.crpt.ru");//СУЗ прод
...
Процедура ПолучитьТокенНаСервере()
	...
	Иначе  
		...
		КодСостоянияHttp = Ответ.КодСостояния;
...
Процедура ОтправитьЗапросНаСервере()
	...
	ОтветНаЗапрос = HTTPОтвет.ПолучитьТелоКакСтроку();
	КодСостоянияОтвета = HTTPОтвет.КодСостояния;
КонецПроцедуры


// ЕНС. Перечитывает сертификаты и обновляет таблицу на форме
&НаСервере
Процедура ОбновитьСертификатыНаСервере()
	...
	// ЕНС. Добавил выбор варианта
	Если ВариантХранилищаСертификатов = 0  Тогда
		oStore.Open(CAPICOM_CURRENT_USER_STORE, CAPICOM_MY_STORE,
			CAPICOM_STORE_OPEN_READ_ONLY); // Открыть хранилище сертификатов
	Иначе 
	...
	Для Каждого ТекСертификат Из oStore.Certificates Цикл  
		...
		Попытка
			НоваяСтрока.Издатель = ТекСертификат.IssuerName;
		Исключение
		    сообщить("Отпечаток: "+ТекСертификат.Thumbprint+" :: ошибка (без прерывания выполнения) (поле объекта не обнаружено): "+ОписаниеОшибки());
		КонецПопытки;
...
Процедура ОбновитьСертификаты(Команда)
	ОбновитьСертификатыНаСервере();
КонецПроцедуры

&НаСервере
Процедура ОчиститьСертификатыНаСервере()
	Сертификаты.Очистить();
КонецПроцедуры

&НаКлиенте
Процедура ОчиститьСертификаты(Команда)
	ОчиститьСертификатыНаСервере();
КонецПроцедуры
