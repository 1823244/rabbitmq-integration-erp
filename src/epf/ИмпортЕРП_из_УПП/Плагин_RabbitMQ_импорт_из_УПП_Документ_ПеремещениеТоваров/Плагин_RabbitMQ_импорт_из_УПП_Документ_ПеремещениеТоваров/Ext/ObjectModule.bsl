...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	Попытка
		...
		СкладПолучательЕРП = ВидОперацииПоСкладуПолучатель.СкладЕРП;
		...
		Если ВидОперацииПоСкладуПолучатель.Отправители.Количество() = 0 Тогда
			...
			мЛоггер.инфо("ВидОперацииПоСкладуПолучатель. Отправители пустые! УПП: Документ: %1, Вид операции: %2", 
				ПредставлениеДокументаУПП, строка(ВидОперацииПоСкладуПолучатель));
		...
		СкладОтправительЕРП = Неопределено;
		Если НЕ ЗначениеЗаполнено(ВидОперацииПоСкладуПолучатель) Тогда
			...
			мЛоггер.варн("%2, %1", 
				ПредставлениеДокументаУПП, т);
		Иначе 
			СкладОтправительЕРП = ВидОперацииПоСкладуОтправитель.СкладЕРП;	
		КонецЕсли;    		
		...
		Если СкладОтправительУПП <> СкладПолучательУПП Тогда
			...
			Для каждого строкаОтправитель Из ВидОперацииПоСкладуПолучатель.Отправители Цикл
				...
				Иначе 
					...
					Если строкаОтправитель.Склад <> СкладОтправительУПП Тогда
						
						ВидДокумента 		= СтруктураОбъекта.type;
						Склад 				= Неопределено;
						СкладОтправитель 	= СкладОтправительУПП;
						СкладПолучатель 	= СкладПолучательУПП;
						т = "Этап 04. Код 425. Склад в ТЧ отфильтрован. Склад в ТЧ: "+строка(строкаОтправитель.Склад)+", Склад отправитель "+строка(СкладОтправительУПП);
						ТекстСообщения 		= т;
						ЛогикаСклад			= Неопределено;
						ЛогикаПеремещения 	= Неопределено;
						Обработчик 			= Неопределено;
				...
				Для каждого строкаЛогики Из Логика.ОбработчикиТиповДокументов Цикл
					...
					Если НЕ строкаЛогики.ВидДокументаУПП = ВидДокументаУППСсылка Тогда
						Продолжить;
					КонецЕсли;
				...
				РезультатЗагрузки = ОбъектОбработчика.ЗагрузитьОбъект(СтруктураОбъекта, jsonText, СкладОтправительЕРП, СкладПолучательЕРП);
				...
				Id.ref);      
					...
					мЛоггер.инфо("Записан Документ! УПП: Документ: %1, ЕРП: %2", 
						ПредставлениеДокументаУПП, строка(РезультатЗагрузки));
