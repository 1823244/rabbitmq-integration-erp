...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	Попытка
		...
		Если НашлиЛогикуОтправителя Тогда
		...
		Если ОбъектОбработчика = Неопределено Тогда
			...
			СкладОтправитель 	= СкладОтправительУПП;
			СкладПолучатель 	= СкладПолучательУПП;
			т = "Этап 04. Код 430. НЕ удалось подключить субплагин импорта! Логика "+строка(ЛогикаПеремещения)+" Субплагин: "+строка(СсылкаОбработчика);
			ТекстСообщения 		= т;
			//ЛогикаПеремещения 	= Неопределено;
			ЛогикаСклад 		= Неопределено;
