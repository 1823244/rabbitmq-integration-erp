...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	Попытка
		...
		Если ВидОперацииПоСкладуОтправитель.Отправители.Количество() = 0 Тогда
			
			мЛоггер.инфо("Отправители пустые! УПП: Документ: %1, ЕРП: %2", 
				ПредставлениеДокументаУПП, строка(ВидОперацииПоСкладуОтправитель));
			
		КонецЕсли;
		
		Если ВидОперацииПоСкладуОтправитель.Получатели.Количество() = 0 Тогда
			
			мЛоггер.инфо("Получатели пустые! УПП: Документ: %1, ЕРП: %2", 
				ПредставлениеДокументаУПП, строка(ВидОперацииПоСкладуОтправитель));
		...
		Если ЗначениеЗаполнено(ЛогикаОтправителя) И
			ЗначениеЗаполнено(ЛогикаПолучателя) Тогда
			
			
			Если ЛогикаОтправителя = ЛогикаПолучателя Тогда
			
				// используем любую
				
				субплагин = НайтиСубПлагинВЛогикеПоВидуДокумента(ЛогикаОтправителя, ВидДокументаУППСсылка);

				млоггер.инфо("Найден субплагин <"+строка(субплагин)+"> при условиях: ЗначениеЗаполнено(ЛогикаОтправителя) И ЗначениеЗаполнено(ЛогикаПолучателя) И ЛогикаОтправителя = ЛогикаПолучателя");
			Иначе 
				// вроде как 2 логики надо обработать.
				// Пока обработаем логику получателя
				
				
				субплагин = НайтиСубПлагинВЛогикеПоВидуДокумента(ЛогикаПолучателя, ВидДокументаУППСсылка);

				млоггер.инфо("Найден субплагин <"+строка(субплагин)+"> при условиях: ЗначениеЗаполнено(ЛогикаОтправителя) И ЗначениеЗаполнено(ЛогикаПолучателя) И ЛогикаОтправителя <> ЛогикаПолучателя. Обработали логику ПОЛУЧАТЕЛЯ");
		...
		Иначе 
			...
			Если НЕ ЗначениеЗаполнено(ЛогикаОтправителя) И НЕ ЗначениеЗаполнено(ЛогикаПолучателя) Тогда
				...
				СкладОтправитель 	= СкладОтправительУПП;
				СкладПолучатель 	= СкладПолучательУПП;
				т = "Этап 04. Код 460. Невозможно найти плагин! Не найдена логика ни в ТЧ Получатели, ни в ТЧ Отправители! ВидОперацииПоСкладуОтправитель = "+строка(ВидОперацииПоСкладуОтправитель)+
					", ВидОперацииПоСкладуПолучатель = "+строка(ВидОперацииПоСкладуПолучатель);
				ТекстСообщения 		= т;
				ЛогикаСклад			= Неопределено;
				ЛогикаПеремещения 	= Неопределено;
				Обработчик 			= Неопределено;
			...
			ИначеЕсли НЕ ЗначениеЗаполнено(ЛогикаОтправителя) И ЗначениеЗаполнено(ЛогикаПолучателя) Тогда
				...
				млоггер.инфо("Найден субплагин <"+строка(субплагин)+"> при условиях: НЕ ЗначениеЗаполнено(ЛогикаОтправителя) И ЗначениеЗаполнено(ЛогикаПолучателя) ");
				
			ИначеЕсли ЗначениеЗаполнено(ЛогикаОтправителя) И НЕ ЗначениеЗаполнено(ЛогикаПолучателя) Тогда
				
				субплагин = НайтиСубПлагинВЛогикеПоВидуДокумента(ЛогикаОтправителя, ВидДокументаУППСсылка);
				
				млоггер.инфо("Найден субплагин <"+строка(субплагин)+"> при условиях: ЗначениеЗаполнено(ЛогикаОтправителя) И НЕ ЗначениеЗаполнено(ЛогикаПолучателя) ");
		...
		Если НЕ ЗначениеЗаполнено(субплагин) Тогда
			...
			СкладОтправитель 	= СкладОтправительУПП;
			СкладПолучатель 	= СкладПолучательУПП;
			т = "Этап 04. Код 420. Не найден субплагин в ТЧ элемента логики! Логика обработки: "+строка(ЛогикаОтправителя);
			ТекстСообщения 		= т;
			ЛогикаСклад			= Неопределено;
			ЛогикаПеремещения 	= ЛогикаОтправителя;
			Обработчик 			= Неопределено;
