...
Функция ОбработкаСитуацииКогдаСкладыОтличаются(ВидОперацииПоСкладуПолучатель, СтруктураОбъекта)
		...
		// сначала найти строку со складом мСкладОтправительУПП
		// если не найдем - ищем с пустым складом (она всего одна)
		
		Логика = Неопределено;
		
		НашлиСтрокуСклада = Ложь;
		Для каждого строкаОтправитель Из ВидОперацииПоСкладуПолучатель.Отправители Цикл
			Если ЗначениеЗаполнено(строкаОтправитель.Склад) и (строкаОтправитель.Склад = мСкладОтправительУПП) Тогда
				НашлиСтрокуСклада = Истина;
				Логика = строкаОтправитель.ЛогикаОбработкиВТЧ;	
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		Если НашлиСтрокуСклада = ЛОЖЬ Тогда
			//ищем строку, где склад не заполнен. она должна быть одна. если их больше - выберем любую одну
			Для каждого строкаОтправитель Из ВидОперацииПоСкладуПолучатель.Отправители Цикл
