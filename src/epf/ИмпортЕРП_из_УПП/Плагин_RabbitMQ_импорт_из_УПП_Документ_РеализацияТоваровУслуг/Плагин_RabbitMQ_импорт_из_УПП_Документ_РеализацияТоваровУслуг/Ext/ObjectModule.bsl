...
Функция ЗагрузитьОбъект(СтруктураОбъекта, пjsonText = "") Экспорт
	...
	_знч = "";
	ЕстьЗначение = деф.ВидыОперацийРеализацияТоваров.свойство("Значение",_знч);
	Если ЕстьЗначение Тогда
		ВидыОперацийРеализацияТоваров = _знч;
	КонецЕсли;
	...
	Если ВидыОперацийРеализацияТоваров = "ПродажаКомиссия"
		И ДоговорКонтрагента.ТипДоговора = Перечисления.ТипыДоговоров.СКомиссионером Тогда
				
		Рез = Схема_1_ПеремещениеНаФулфилмент(СтруктураОбъекта);
		
	Иначе
		
		Рез = Схема_2_РеализацияТоваров(СтруктураОбъекта);
...
Функция СоздатьЗаказНаПеремещение(СтруктураОбъекта, СуществующийЗаказСсылка)
	...
	Для счТовары = 0 По деф.ТЧТовары.Количество()-1 Цикл
		...
		СтрокаТЧ.Характеристика = ксп_ИмпортСлужебный.НайтиХарактеристика(стрк.ХарактеристикаНоменклатуры);
...
Функция СоздатьРасходныйОрдер(СтруктураОбъекта, СуществующийРОСсылка, ЗаказНаПеремещениеОбъект)
	...
	//КонецЕсли;
	//// на случай, если есть метод поиска ссылки:
	//ОбъектДанных.СкладскаяОперация = ксп_ИмпортСлужебный.НайтиПеречисление_СкладскаяОперация(деф.СкладскаяОперация);
...
Функция СоздатьПеремещение(СтруктураОбъекта, СуществующийПеремещениеСсылка, РООбъект, ЗаказНаПеремещениеОбъект)
	...
	//гуид="";
	//ЕстьАтрибут = деф.ЗаказНаПеремещение.свойство("Ref",гуид);
	//Если ЕстьАтрибут Тогда
	//	ОбъектДанных.ЗаказНаПеремещение = Справочники_Документы.КакойТоВидМД.ПолучитьСсылку( Новый УникальныйИдентификатор( деф.ЗаказНаПеремещение.Ref ) );
	//Иначе
	//	ОбъектДанных.ЗаказНаПеремещение = Неопределено;
	//КонецЕсли;
	//// на случай, если есть метод поиска ссылки:
	//ОбъектДанных.ЗаказНаПеремещение = ксп_ИмпортСлужебный.НайтиЗаказНаПеремещение(деф.ЗаказНаПеремещение);
	...
	//ОбъектДанных.ПеремещениеПоЗаказам = деф.ПеремещениеПоЗаказам;
...
Функция Схема_2_РеализацияТоваров(СтруктураОбъекта) Экспорт
