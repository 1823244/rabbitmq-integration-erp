...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	//ЕНС. 16.03.2024 Нет Такого реквизита в отчете о розн продажах
	//СкладХраненияУПП = ПолучитьСсылкуСправочникаПоДаннымID(деф.СкладХранения, "КСП_СкладыХраненияУПП");
	//++ Задача 18036 Оздьон ЕВ 14.03.2024  
	...
	//КонецЕсли;
	//-- Задача 18036
#Область ЛогикаОбработкиДокумента	
...
#КонецОбласти	
	...
	СсылкаОбработчика = Справочники.ДополнительныеОтчетыИОбработки.НайтиПоНаименованию("СубПлагин_импорт_ОтчетОРозничныхПродажах", ИСтина);	
		Если НЕ ЗначениеЗаполнено(СсылкаОбработчика) Тогда
			ВидДокумента 		= СтруктураОбъекта.type;
			Склад 				= СкладУПП;
			СкладОтправитель 	= Неопределено;
			СкладПолучатель 	= Неопределено;
			ТекстСообщения 		= "Не найден субплагин в Справочнике доп обработок!";
			ЛогикаПеремещения 	= Неопределено;
			ЛогикаСклад 		= Неопределено;
			Обработчик 			= Неопределено;
		НомерДокумента 		= деф.Number;
