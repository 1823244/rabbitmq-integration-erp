...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
    ...
    Попытка
		...
		Для каждого ВидОперацииПоСкладу Из МассивВидОперацииПоСкладу Цикл
			...
			ЛогикаОбработки = НайтиЛогикуПоВидуДокумента(ВидОперацииПоСкладу.Получатели, СкладХраненияУПП, ВидДокументаУППСсылка);	
		...
		мЛоггер.инфо("Плагин: Импорт Документ.КомплектацияНоменклатуры. Из документа УПП № %1 от %2 создан документ ЕРП 'Сборка товаров' № %3 от %4",
			деф.Number,Строка(деф.Date),Рез.СборкаТоваров.Номер,Рез.СборкаТоваров.Дата);
		
		Возврат  Рез;
           
    Исключение
        мЛоггер.ерр("Плагин: Импорт Документ.КомплектацияНоменклатуры. Подробности: "+ОписаниеОшибки());
...
// Ищет в ТЧ Получатели/Отправители "логику" с учетом вида документа
//
// Параметры:
...
//
Функция НайтиЛогикуПоВидуДокумента(ТЧПолучателиОтправители, СкладХраненияУПП, ВидДокументаУППСсылка)
	
	ЛогикаОбработки = Неопределено;
	
	//ЕНС. Найти все строки в ТЧ, где есть искомый вид документа в "логике" (там это ТЧ ОбработчикиТиповДокументов)
