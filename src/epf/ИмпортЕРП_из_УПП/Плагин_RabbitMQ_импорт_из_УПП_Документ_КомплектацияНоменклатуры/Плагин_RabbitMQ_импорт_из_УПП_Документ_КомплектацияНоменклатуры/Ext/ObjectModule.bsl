...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
    ...
    Попытка
		...
		мЛоггер.инфо("Из документа УПП № %1 от %2 создан документ ЕРП № %3 от %4",
			деф.Number,Строка(деф.Date),Рез.СборкаТоваров.Номер,Рез.СборкаТоваров.Дата);
		
		Возврат  Рез;
           
    Исключение
        мЛоггер.ерр("Плагин: Плагин_RabbitMQ_импорт_из_УПП_Документ_ОтчетКомиссионераОПродажах . Подробности: "+ОписаниеОшибки());
	КонецПопытки;
