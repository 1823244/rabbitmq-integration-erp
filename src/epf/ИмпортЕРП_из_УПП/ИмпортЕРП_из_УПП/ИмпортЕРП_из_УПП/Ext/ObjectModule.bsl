...
Процедура ВыполнитьИмпортДокументов() Экспорт
	...
	ЗагрузитьДанныеИзОчереди("to-erp-from-upp-doc");
...
Функция НайтиПлагиныВДопОбработках()
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	спр.ИмяОбъекта ПОДОБНО ""Плагин_RabbitMQ_импорт_из_УПП%""  И спр.ПометкаУдаления = ЛОЖЬ";
	...
	Попытка
		...
		Пока Клиент.BasicConsumeMessage("", ПолучаемоеСообщение, ТегСообщения, 5000) Цикл
			...
			Попытка
				...
				//Если тДанные.ТИпДокумент = "ВводОстатков" Тогда
				//	// это вообще не документ
				//	
				//ИначеЕсли тДанные.ТИпДокумент = "Перемещение" Тогда
				//	
				//	//Сгруппировать по складу отправителю из ТЧ Товары
				//	
				//	Для каждого СкладОтпр Из КоллекцияСкладовОтпр Цикл
				//			
				//		ВидОпер = Найти_КСП_ВидыОперацийПоСкладамУПП(тДанные.Склад);
				//		
				//		Если видОпер.ЛогикаОбработкиВШапке = Справочники.КСП_ЛогикаОбработкиДвиженияПоСкладуУПП.Логика1 Тогда
				//			
				//			// заранее отобрать товары по складу  СкладОтпр  (или в логике?)
				//			
				//			ТоварыПоСкладу = ...
				//			
				//			ВыполнитьЛогику1(тДанные, ТоварыПоСкладу); //Заказ поставщику + ПО+Заказ (резерв по каналу)
				...
				//	
				//	
				//ИначеЕсли тДанные.ТИпДокумент = "РеализацияТоваровУслуг" Тогда
				//	
				//	ВнешняяСистема = "UPP";
				//	ГуидКонтрагентаУПП = тДанные.КОнтрагент.гуид;
				//	Запись = РС.ксп_КлассификацияРеализацийТоваровУППпоКонтрагентам.Найти(ВнешняяСистема, ГуидКонтрагентаУПП);	
				//			
				//	 Если Запись.ТипРеализации = перечисления.ксп_КлассыРеализацийТоваровУППпоКонтрагентам.Интеркампани Тогда 
				//		 // создаем Передача между организациями
				//		 
				//	 ИначеЕсли Запись.ТипРеализации = перечисления.ксп_КлассыРеализацийТоваровУППпоКонтрагентам.Фулфилмент Тогда 
				//		 
				//		 //создаем перемещение
				//		 
				//	 Иначе 
...
//это для импорта статики. там нет сложных схем
Процедура ЗагрузитьДанныеИзОчереди_static2(ИмяОчереди, Клиент = Неопределено) Экспорт
