...
Процедура ВыполнитьИмпортДокументов(Знач ИдВызова = Неопределено) Экспорт
	...
	мИдВызова = ИдВызова;//пока не нужен, но на случай параллельного импорта понадобится
	...
	Пока мКлиент.BasicConsumeMessage(consumerId, ПолучаемоеСообщение, мТегСообщения, 3000) Цикл
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(ИмяСобытияЖР,УровеньЖурналаРегистрации.Информация,,,
				"Обработка сообщения с тэгом "+мТегСообщения+". Тип объекта = "+Строка(ТипОбъекта)+". Сессия ИД "+мИДСессии);
		Исключение                                                                 
			т = ОписаниеОшибки();
			ЗаписьЖурналаРегистрации(ИмяСобытияЖР,УровеньЖурналаРегистрации.Ошибка,,,
            	"Удалено из очереди сообщение_ с тэгом "+мТегСообщения+". Ошибка чтения json: "+Строка(т)+". Сессия ИД "+мИДСессии);
		...
		Попытка	
			...
			ЗаписьЖурналаРегистрации(ИмяСобытияЖР,УровеньЖурналаРегистрации.Информация,,,
				"Получено сообщение с тэгом <"+Строка(мТегСообщения)+">. Сессия ИД "+мИДСессии);
		...
		Исключение
			...
			Если НЕ ЗначениеЗаполнено(т) Тогда
				...
				т = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(ИмяСобытияЖР,УровеньЖурналаРегистрации.Ошибка,,,
					"Ошибка обработки (Не в PinkRabbitMQ.dll) сообщения с тэгом <"+Строка(мТегСообщения)+">. Сессия ИД "+мИДСессии+". Подробности: "+т);
			Иначе 
				ЗаписьЖурналаРегистрации(ИмяСобытияЖР,УровеньЖурналаРегистрации.Ошибка,,,
					"Ошибка PinkRabbitMQ.dll при обработке сообщения с тэгом <"+Строка(мТегСообщения)+">. Сессия ИД "+мИДСессии+". Подробности: "+т);
			КонецЕсли; 
