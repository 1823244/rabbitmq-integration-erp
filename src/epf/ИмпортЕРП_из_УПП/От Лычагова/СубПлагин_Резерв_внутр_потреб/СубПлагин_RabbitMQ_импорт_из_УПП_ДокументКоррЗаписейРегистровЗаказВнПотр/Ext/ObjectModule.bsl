...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	Если Лев(деф.Комментарий,15) = "остатки для ERP" и деф.DeletionMark Тогда
		СсылкаВводОстатков = ЗагрузкаЗаказНаВнутреннееПотребление(id,деф);
...
Процедура ЗаполнитьДокументЗаказНаВнутреннееПотребление(ОбъектДанных, деф)
...
Функция ЗагрузкаЗаказНаВнутреннееПотребление(id, деф, ВидОбъекта = "ЗаказНаВнутреннееПотребление")
	ИмяСобытияЖР = "Импорт_из_RabbitMQ_УПП";
	...
	Провести = ложь;
	Попытка			
		ЗаполнитьДокументЗаказНаВнутреннееПотребление(ОбъектДанных, деф);		
