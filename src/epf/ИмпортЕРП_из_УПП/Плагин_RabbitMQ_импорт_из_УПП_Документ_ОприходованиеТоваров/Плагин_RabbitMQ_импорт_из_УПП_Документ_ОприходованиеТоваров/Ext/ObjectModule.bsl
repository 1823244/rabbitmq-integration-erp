...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	Попытка
		...
		Если НЕ ЗНачениезаполнено(СкладУПП) Тогда
			...
			мЛоггер.Дебаг("Документ не загружен! Не найден склад УПП для Оприходования : %1", 
		...
		ИначеЕсли 
			...
			мЛоггер.Дебаг("Документ не загружен! Не найден склад ХРАНЕНИЯ УПП для Оприходования : %1", 
				ПредставлениеДокументаУПП);
		...
		Если НЕ ЗНачениезаполнено(ВидДокументаУППСсылка) Тогда
			...
			мЛоггер.Дебаг("Документ не загружен! Не найден Вид документа УПП Оприходование : %1", 
				ПредставлениеДокументаУПП);
		...
		Если МассивВидОперацииПоСкладу.Количество() = 0 Тогда
			...
			Ложь,
			Id.ref);
				мЛоггер.Дебаг("Документ не загружен! Не найден Вид операции УПП : %1", 
				ПредставлениеДокументаУПП);
		...
		Для каждого ВидОперацииПоСкладу Из МассивВидОперацииПоСкладу Цикл
			...
			Если НЕ ЗначениеЗаполнено(ЛогикаОбработки) Тогда
				...
				Ложь,
				Id.ref);
				мЛоггер.Дебаг("Документ не загружен! Не найдена Логика обработки УПП : %1, СкладУПП : %2, ВидОперацииПоСкладу : %3", 
			...
			Если НЕ ЗначениеЗаполнено(СсылкаОбработчика) Тогда
				...
				Ложь,
				Id.ref);
				мЛоггер.Дебаг("Документ не загружен! Не найден Субплагин : %1", 
				ПредставлениеДокументаУПП);
