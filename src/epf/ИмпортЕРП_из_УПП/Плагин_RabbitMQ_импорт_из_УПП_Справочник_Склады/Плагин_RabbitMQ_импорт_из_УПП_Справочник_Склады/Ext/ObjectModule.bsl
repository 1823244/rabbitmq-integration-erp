...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Версия","1.5.3");
...
Функция Загрузить_Справочник_КСП_СкладыУПП(СтруктураОбъекта, jsonText = "") Экспорт
	...
	Если РегистрыСведений[ВидРегистраМэппинга].ЕстьГУИД(id.Ref, мВнешняяСистема) Тогда
		...
		СкладУПП = Справочники.ксп_СкладыУПП.ПолучитьСсылку(Новый УникальныйИдентификатор(id.Ref));
		Склад = Справочники.КСП_ВидыОперацийПоСкладамУПП.ПоМэппингу(СкладУПП);
		//Склад = ксп_ИмпортСлужебный.НайтиСклад(id.ref, мВнешняяСистема);
    	Сообщить("Уже есть в Мэппинге: " + Строка(Склад));
	...
	Иначе 
    	...
    	Сообщить("Добавим в Мэппинге:" + деф.Description);
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ДанныеСсылка.ВерсияДанных) Тогда 
    	Сообщить("Новый элемент в спр-ке");
	...
	Сообщить("Обновили данные:" + НаименованиеДляМэппинга + " - " + мВнешняяСистема +  " - " + Склад);
