...
Функция ЗагрузитьОбъект(СтруктураОбъекта) Экспорт
	...
	Попытка
		...
		Если Менеджер <> Неопределено Тогда
			Если НЕ ЗначениеЗаполнено(Менеджер.ВерсияДанных) Тогда
				ОбъектДанныхМенеджер = Справочники.Пользователи.СоздатьЭлемент();
				СсылкаНовогоМенеджер = Справочники.Пользователи.ПолучитьСсылку(Новый УникальныйИдентификатор(def.Менеджер.ref));
				ОбъектДанныхМенеджер.УстановитьСсылкуНового(СсылкаНовогоМенеджер);
				ОбъектДанныхМенеджер.Наименование = def.Менеджер.description;
			Иначе 
				ОбъектДанныхМенеджер = СуществующийОбъект.ПолучитьОбъект();	
			КонецЕсли;
		КонецЕсли;
