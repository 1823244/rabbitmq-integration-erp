...
//	Файл (File)
	...
	Для каждого ОписаниеПомещенногоФайла Из МассивФайлов Цикл
		...
		ЗагрузитьИзJsonНаСервереИзКаталога(МассивФайлов);
	КонецЦикла;
	
КонецПроцедуры   

&НаСервере
Процедура ЗагрузитьИзJsonНаСервереИзКаталога(МассивФайлов)
	
	обк=РеквизитФормыВЗначение("Объект");
	Для каждого ОписаниеПомещенногоФайла Из МассивФайлов Цикл
		обк.ЗагрузитьИзJsonНаСервереИзФайла(ОписаниеПомещенногоФайла.Адрес, ВидНоменклатуры);
	КонецЦикла;
