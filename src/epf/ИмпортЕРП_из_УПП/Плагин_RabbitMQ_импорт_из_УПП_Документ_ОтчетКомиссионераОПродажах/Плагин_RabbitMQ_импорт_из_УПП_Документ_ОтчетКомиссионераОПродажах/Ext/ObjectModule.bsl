...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	////		ДОКУМЕНТ С ОДНИМ СКЛАДОМ
	
	схема = найтиСхему(деф.Склад);
	
	строкаТЧ = схема.ЛогикаОбработкиВШапке.Обработчики.найти(
		Перечисление.ОтчетКомиссионераОПРодажах, "ТипДокументаУПП");
		
		
	Если СтрокаТЧ = Неопределено Тогда
		записать ошику в регистр, который еще надо создать
		Выход
	КонецЕсли;

	ОбъектПлагина 	= ДополнительныеОтчетыИОбработки.ОбъектВнешнейОбработки(строкаТЧ.СсылкаНаДопОбработку);
	
		
	ОбъектПлагина.ЗАгрузитьОбъект(СтруктураОбъекта);
	//////////////////////////////////////////////////////////////////////////
	
	////		ПЕРЕМЕЩЕНИЕ
	
	схемаО = найтиСхему(деф.СкладОтправитель);
	схемаП = найтиСхему(деф.СкладПолучатель);
	
	Логика = Справочники.КСП_ЛогикаОбработкиПеремещений.НАйтиЛогикуПеремещения(СхемаО.ЛогикаОбработкиВШапке, СхемаП.ЛогикаОбработкиВШапке);
	
	Если Логика = Неопределено Тогда
		
		НовыйЭл = Сравочники.КСП_ЛогикаОбработкиПеремещений.СоздатьЭлемент();
		НовыйЭл.ЛогОтпр = СхемаО.ЛогикаОбработкиВШапке;
		НовыйЭл.ЛогПолуч = СхемаП.ЛогикаОбработкиВШапке;
		
		НовыйЭл.наим = "фыва";
		НовыйЭл.записать();
		
		записать событие в регстр, который еще предстоит создать "Создали новую логик для перемещ, выберите обработчик - субплагин!"
		
	Иначе 
			
		
		строкаТЧ = Логика.Обработчики.найти(
			Перечисление.ОтчетКомиссионераОПРодажах, "ТипДокументаУПП");
			
			
		Если СтрокаТЧ = Неопределено Тогда
			записать ошику в регистр, который еще надо создать "Для Перемещения не найдено логики по складам: Отпр , Получ"
			Выход
		КонецЕсли;
