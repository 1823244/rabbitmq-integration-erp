...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	Для каждого ВидОперацииПоСкладу Из МассивВидОперацииПоСкладу Цикл
		...
		Попытка
			...
			РегистрОшибокУПП_ОтметитьИсправление(СтруктураОбъекта.type, деф.Number, деф.Date);
...
//todo перенести в общий модуль СлужебныеИмпорт
// Включает флаг по всем записям регистра с указанным документом УПП
Процедура РегистрОшибокУПП_ОтметитьИсправление(ВидДокумента, 
	НомерДокумента = "",
	...
	Для каждого стрк Из НЗ Цикл
		...
		МЗ = РегистрыСведений.КСП_УПП_ОшибкиИмпорта.СоздатьМенеджерЗаписи();
		ЗаполнитьЗначенияСвойств(МЗ, стрк);
		МЗ.ОшибкаИсправлена = Истина;
		МЗ.Записать();
