		...
		Для счТовары = 0 По деф.ТЧТовары.Количество()-1 Цикл
			...
			Если СобиратьНенайденнуюНоменклатуру Тогда
				Если ТипЗнч(_Номенклатура) = Тип("СправочникСсылка.Номенклатура") И НЕ ЗначениеЗаполнено(_Номенклатура.ВерсияДанных) Тогда
					...
					Если стрк.Номенклатура.Свойство("ref")
						И мНеНайденнаяНоменклатураМассив.Найти(стрк.Номенклатура.ref) = Неопределено Тогда
						мНеНайденнаяНоменклатураМассив.Добавить(стрк.Номенклатура.Ref);
					Иначе 
						мЛоггер.ерр("нет свойства Ref у Номенклатуры! Документ: "+ПредставлениеОбъекта);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
