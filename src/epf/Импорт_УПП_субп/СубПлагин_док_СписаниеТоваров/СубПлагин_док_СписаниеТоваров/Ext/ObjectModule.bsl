...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "", СкладЕРП = Неопределено) Экспорт
	...
	мЛоггер.инфо("Субплагин. Импорт док. УПП Списание товаров №"+СтруктураОбъекта.definition.Number);
	...
	Попытка
		...
		КонецЕсли;
		Если НЕ НРег(СтруктураОбъекта.type) = НРег("СписаниеТоваров") Тогда
			Возврат Неопределено;
		КонецЕсли;
