...
Функция ЗагрузитьОбъект(СтруктураОбъекта, пjsonText = "", СкладЕРП = Неопределено) Экспорт
	...
	Рез = Схема_ПростаяПродажа(СтруктураОбъекта);	
	...
	Если НеНайденнаяНоменклатураМассив.Количество()>0 Тогда
		...
		Исключение
		    ...
		    ЗаписьЖурналаРегистрации("ИмпортИзУПП", УровеньЖурналаРегистрации.Предупреждение,,,т);
...
Функция Схема_ПростаяПродажа(СтруктураОбъекта) Экспорт
	...
	Попытка			
		...
		РегистрыСведений.КСП_СвязьРеализацийУППиЦепочекРеализаций.ДобавитьЗапись(
			мВнешняяСистема, 
			id.Ref, 
	...
	Исключение
		т = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		тт = "Объект не загружен! Ошибка в процессе загрузки документа " + ДокументИзУПП + ". Подробности: " + т;
		ЗаписьЖурналаРегистрации(ИмяСобытияЖР,УровеньЖурналаРегистрации.Ошибка,,,тт);
...
//	ОбъектДанных.АдресДоставкиПеревозчикаЗначенияПолей = деф.АдресДоставкиПеревозчикаЗначенияПолей;   
	...
	ОбъектДанных.Комментарий = "[УПП РеализацияТоваровУслуг №" + деф.Number + " от " + деф.Date + " ]. Оригинальный коммент.: "+деф.Комментарий;
...
Функция ПолучитьБанковскийСчетОрганизации(Организация)
