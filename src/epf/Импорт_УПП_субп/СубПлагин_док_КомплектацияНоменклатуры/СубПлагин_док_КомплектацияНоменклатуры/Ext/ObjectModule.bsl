...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "", СкладЕРП = Неопределено ) Экспорт
    ...
    мЛоггер = мис_ЛоггерСервер.getLogger(мИдВызова, "Субплагин Комплектация номенклатуры");
     
    Попытка
		
		мЛоггер.инфо("Субплагин Комплектация номенклатуры - Запуск. Номер "+строка(СтруктураОбъекта.definition.Номер));
		...
		мЛоггер.инфо("Субплагин Комплектация номенклатуры - Завершение. Номер "+строка(СтруктураОбъекта.definition.Номер));

		Возврат Рез;
           
    Исключение
        мЛоггер.ерр("Субплагин Комплектация номенклатуры - Ошибка. Номер "+строка(СтруктураОбъекта.definition.Номер)+". Подробности: "+ОписаниеОшибки());
	КонецПопытки;
