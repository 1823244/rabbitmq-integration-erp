...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	Если Лев(деф.Комментарий,15) = "остатки для ERP" и деф.DeletionMark Тогда
		СсылкаДок = ЗагрузкаОстатков(id,деф);
		
	Иначе
		СсылкаДок = ЗагрузкаКорректировкаЗаписейРегистров(id,деф);
		
	КонецЕсли;        
	
	// ненайденная номенклатура
...
Процедура ЗаполнитьДокументВводОстатковТоваров(ОбъектДанных, деф)
	ОбъектДанных.Организация = Справочники.Организации.НайтиПоКоду("000001");
	ОбъектДанных.Дата = деф.Date;
	ОбъектДанных.Номер = деф.Number;
	ОбъектДанных.ПометкаУдаления = Ложь;
	ОбъектДанных.Валюта	= Константы.ВалютаРегламентированногоУчета;
	...
	Для каждого строка из деф.РегистрНакопления_ТоварыНаСкладах Цикл
		...
		Если ЗаполненаСсылка(строка.Номенклатура) Тогда
			_Номенклатура = ксп_ИмпортСлужебный.НайтиНоменклатуру(строка.Номенклатура);
			...
			Если ТипЗнч(_Номенклатура) = Тип("СправочникСсылка.Номенклатура") И 
	        	...
	        	Если мНеНайденнаяНоменклатураМассив.Найти(строка.Номенклатура.Ref) = Неопределено Тогда
					мНеНайденнаяНоменклатураМассив.Добавить(строка.Номенклатура.Ref);
				КонецЕсли;
...
Функция ЗагрузкаОстатков(id, деф, ВидОбъекта = "ВводОстатковТоваров")
	ИмяСобытияЖР = "Импорт_из_RabbitMQ_УПП";
...
Функция ЗагрузкаКорректировкаЗаписейРегистров(id, деф, ВидОбъекта = "КорректировкаРегистров")
	Возврат Неопределено;	
КонецФункции
