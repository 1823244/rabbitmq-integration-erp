...
Процедура ЗагрузитьВсеНаСервере(Знач ИдВызова,ИмяСервера,ИмяБазыДанных,
	...
	Если МестоЗапуска = 0 Тогда 
		// ноут XPS
		queryText = "select 
		|cast(_FLD35039RRef as uniqueidentifier) as Goods, 
	...
	ИначеЕсли МестоЗапуска = 1 Тогда
		// УПП
		queryText = "select TOP 3500
		|cast(_Fld15502_RRRef as uniqueidentifier) as Goods, 
		|cast(_Fld15504RRef as uniqueidentifier)  as Chars, 
