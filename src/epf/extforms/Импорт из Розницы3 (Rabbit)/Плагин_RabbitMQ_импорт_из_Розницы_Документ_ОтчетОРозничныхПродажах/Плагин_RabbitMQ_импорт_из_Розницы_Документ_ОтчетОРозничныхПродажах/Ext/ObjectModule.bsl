...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	мДокументДляОповещения = ДокументДляОповещения;
	
	мЛоггер.инфо("Импорт документа (начало): %1", ДокументДляОповещения);
	...
	Попытка			
		...
		Если ЗначениеЗаполнено(ОбъектДанных.Дата) Тогда
		...
		Иначе 
			...
			// в отчете не было продаж, а только возвраты и наш алгоритм его не заполнил
			// поэтому записывать не надо
			
			мЛоггер.инфо("Документ Отчет о розн. продажах не создан, т.к. в исходном документе не было строк с продажами: %1. ", ДокументДляОповещения);
		...
		мЛоггер.инфо("Импорт документа (завершение): %1", ДокументДляОповещения);
...
Процедура СоздатьЗаполнитьВозвратПодарочныхСертификатов(СтруктураОбъекта, Таблицы)
	...
	мЛоггер.инфо("Импорт документа: %1. Создан документ: %2", мДокументДляОповещения, ДокументРеализацииВозврат.Ссылка);
...
Процедура СоздатьЗаполнитьОтчетОРозничныхВозвратах(СтруктураОбъекта, Таблицы)  
	...
	мЛоггер.инфо("Импорт документа: %1. Создан документ: %2", мДокументДляОповещения, ОтчетОРозничныхВозвратах.Ссылка);
...
Процедура СоздатьЗаполнитьВнутренннееПотребление(СтруктураОбъекта, Таблицы) 
	...
	мЛоггер.инфо("Импорт документа: %1. Создан документ: %2", мДокументДляОповещения, ВнутреннееПотребление.Ссылка);
		...
		Если Число(Строка.Возврат) > 0 Тогда 
			...
			Если Строка.Номенклатура.Свойство("Номинал") Тогда 
				...
				Если НЕ ЗначениеЗаполнено(СтруктураСертификата) Тогда 
					...
					ВызватьИсключение "Не удалось найти подарочный сертификат №"+строка(НомерКарты);
