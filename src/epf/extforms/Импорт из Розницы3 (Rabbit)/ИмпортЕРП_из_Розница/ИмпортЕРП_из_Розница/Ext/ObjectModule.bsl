...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование","Импорт из 1С:Розница (Реконнект в цикле)");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация","Импорт из 1С:Розница (Реконнект в цикле)");
	...
	ДобавитьКоманду(ПараметрыРегистрации.Команды, "Открыть форму : Импорт из 1С:Розница","ЗагрузитьДанныеИз1СРозницаИнтерактивно",ТипКоманды, Ложь) ;
	
	ТипКоманды = ДополнительныеОтчетыИОбработкиКлиентСервер.ТипКомандыВызовСерверногоМетода();
	ДобавитьКоманду(ПараметрыРегистрации.Команды, "Выполнить импорт НСИ из 1С:Розница","ЗагрузитьНСИИз1СРозница",ТипКоманды, Ложь) ;
	
	ТипКоманды = ДополнительныеОтчетыИОбработкиКлиентСервер.ТипКомандыВызовСерверногоМетода();
	ДобавитьКоманду(ПараметрыРегистрации.Команды, "Выполнить импорт документов из 1С:Розница","ЗагрузитьДокументыИз1СРозница",ТипКоманды, Ложь) ;
...
Процедура ВыполнитьКоманду(ИмяКоманды, ПараметрыВыполнения) Экспорт
	...
	Если ИмяКоманды = "ЗагрузитьДокументыИз1СРозница" Тогда
		ВыполнитьИмпортДокументов();
	ИначеЕсли ИмяКоманды = "ЗагрузитьНСИИз1СРозница" Тогда
		ВыполнитьИмпортНСИ();
	КонецЕсли;
...
Процедура ВыполнитьИмпортДокументов(Знач ИдВызова = Неопределено) Экспорт
	...
	мИдВызова = ИдВызова;
	мЛоггер = мис_ЛоггерСервер.getLogger(ИдВызова, "Импорт документов из Розница");
	
	// на случай, если в расширении RabbitConnector нет констант (Старая версия)
	Очередь = "to-erp-from-retail-doc";
...
Процедура ВыполнитьИмпортНСИ(Знач ИдВызова = Неопределено) Экспорт
	...
	мЛоггер = мис_ЛоггерСервер.getLogger(ИдВызова, "Импорт НСИ из Розница");
	
	// на случай, если в расширении RabbitConnector нет констант (Старая версия). !!
	
	Очередь = "to-erp-from-retail-static";
...
Функция НайтиПлагиныВДопОбработках()
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	спр.ИмяОбъекта ПОДОБНО ""Плагин_RabbitMQ_импорт_из_Розницы%""
