...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование","Плагин_RabbitMQ_импорт_из_Розницы_Справочник_СпособыОплаты");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация","Плагин_RabbitMQ_импорт_из_Розницы_Справочник_СпособыОплаты");
	...
	ДобавитьКоманду(ПараметрыРегистрации.Команды, 
		"Открыть форму : Плагин_RabbitMQ_импорт_из_Розницы_Справочник_СпособыОплаты",
...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	Если НЕ НРег(СтруктураОбъекта.type) = НРег("справочник.СпособыОплаты") Тогда
    ...
    ВидОбъекта = "ЭквайринговыеТерминалы"; 
	...
	Если НЕ ЗначениеЗаполнено(ДанныеСсылка.ВерсияДанных) Тогда
		...
		ПредставлениеОбъекта = "Эквайринговый терминал (новый)";
		ЭтоНовый = Истина;
	КонецЕсли; 
...
Функция ЗаполнитьРеквизиты(ОбъектДанных, СтруктураОбъекта, jsonText = "") Экспорт
	...
	ДоговорЭквайринга = СоздатьДоговорЭкваинга(СтруктураОбъекта, jsonText);
	...
	ОбъектДанных.Владелец = ДоговорЭквайринга;
...
Функция СоздатьДоговорЭкваинга(СтруктураОбъекта, jsonText = "") 

	ИмяСобытияЖР = "Импорт_из_RabbitMQ_Розница";

	id = СтруктураОбъекта.identification;
	деф = СтруктураОбъекта.definition;
    ВидОбъекта = "ДоговорыЭквайринга"; 
	...
	Если НЕ ЗначениеЗаполнено(ДанныеСсылка.ВерсияДанных) Тогда
		...
		СсылкаНового = Справочники[ВидОбъекта].ПолучитьСсылку(Новый УникальныйИдентификатор(id.Ref));
		ОбъектДанных.УстановитьСсылкуНового(СсылкаНового);
		ПредставлениеОбъекта = "Договор эквайринга";
		ЭтоНовый = Истина;
	КонецЕсли; 
	...
	ЗаполнитьРеквизитыДоговора(ОбъектДанных, СтруктураОбъекта, jsonText);
...
Процедура ЗаполнитьРеквизитыДоговора(ОбъектДанных, СтруктураОбъекта, jsonText = "") 
    ...
    ОбъектДанных.БанковскийСчет = ксп_ИмпортСлужебный.НайтиБанковскийСчет(ксп_ИмпортСлужебный.ПолучитьСвойство(Деф,"БанковскийСчетЭквайринг.НомерСчета"),
																	 ксп_ИмпортСлужебный.ПолучитьСвойство(Деф,"БанковскийСчетЭквайринг.БИК")); 
																	 
	ОбъектДанных.Наименование = деф.Description;
	ОбъектДанных.ПометкаУдаления = деф.DeletionMark;																 
																	 
	ОбъектДанных.Контрагент  = ксп_ИмпортСлужебный.НайтиКонтрагента(деф.Эквайрер, мВнешняяСистема);
	
	ОбъектДанных.Партнер  = ксп_ИмпортСлужебный.ПолучитьСвойство(ОбъектДанных.Контрагент,"Партнер"); 
	
	ОбъектДанных.Статус   =  перечисления.СтатусыДоговоровКонтрагентов.Действует;
	
	ОбъектДанных.Согласован = Истина;
	
	ОбъектДанных.ИспользуютсяЭквайринговыеТерминалы = Истина;
	
	ОбъектДанных.ДетальнаяСверкаТранзакций  = Ложь;
	
	ОбъектДанных.РазрешитьПлатежиБезУказанияЗаявок = Истина;
	
	ОбъектДанных.СпособОтраженияКомиссии = Перечисления.СпособыОтраженияЭквайринговойКомиссии.ВоВремяТранзакции; 
	
	ОбъектДанных.СпособПроведенияПлатежа = ксп_ИмпортСлужебный.КонвертацияСпособыПроведенияПлатежей(Деф.Тип);
