...
Процедура ВыполнитьКоманду(ИдентификаторКоманды, ОбъектыНазначения)Экспорт 
	...
	Для каждого стрк Из ВладелецФормы.Объект.Товары Цикл
		Если ВладелецФормы.Элементы.Товары.ТекущиеДанные.НомерСтроки = стрк.НомерСтроки Тогда
			Если ЗначениеЗаполнено(стрк.КСП_ПроверочнаяВедомость) Тогда  
				ПоказатьЗначение(,стрк.КСП_ПроверочнаяВедомость);
			Иначе    
				стрк.КСП_ПроверочнаяВедомость = СоздатьПроверочнуюВедомость(ВладелецФормы.Объект.Ссылка,ВладелецФормы.Объект.Распоряжение,стрк.Номенклатура, стрк.Количество, 
				стрк.Упаковка, стрк.Характеристика, стрк.Серия ,стрк.СтатусУказанияСерий);		
