...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Версия","1.29");
...
Функция СоздатьЭтап(СтруктураОбъекта, ЗаказНаПроизводствоОбъект, СуществующийЭтапСсылка)
	...
	Действие = ДействиеСДокументом(ЭтоНовый, СуществующийЭтапСсылка, деф);
	Если Действие = НЕ_ЗАГРУЖАТЬ Тогда
		ОтменитьТранзакцию();
		мЛоггер.инфо("Действие = НЕ Загружать. Документ пропущен: %1", ДокументИзУПП);
		Возврат ОбъектДанных;
...
Функция СоздатьДвижениеПродукцииИМатериалов(СтруктураОбъекта, ЗаказНаПроизводствоОбъект, ЭтапОбъект, СуществующийДвижениеПродукцииСсылка, СкладЕРП)
	...
	Действие = ДействиеСДокументом(ЭтоНовый, СуществующийДвижениеПродукцииСсылка, деф);
	Если Действие = НЕ_ЗАГРУЖАТЬ Тогда
		ОтменитьТранзакцию();
		мЛоггер.инфо("Действие = НЕ Загружать. Документ пропущен: %1", ДокументИзУПП);
		Возврат ОбъектДанных;
...
Функция СоздатьЗаказНаПеремещение(СтруктураОбъекта, СуществующийЗаказНаПеремещениеСсылка, СкладЕРП, 
	...
	Действие = ДействиеСДокументом(ЭтоНовый, СуществующийЗаказНаПеремещениеСсылка, деф);
	Если Действие = НЕ_ЗАГРУЖАТЬ Тогда
		ОтменитьТранзакцию();
		мЛоггер.инфо("Действие = НЕ Загружать. Документ пропущен: %1", ДокументИзУПП);
		Возврат ОбъектДанных;
