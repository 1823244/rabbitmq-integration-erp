...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Версия","1.30");
...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "", СкладЕРП = Неопределено ) Экспорт
	...
	мЛоггер = Вычислить("КСП_ЛоггерСервер.getLogger(мИдВызова, ""Импорт документов из УПП: СписаниеТоваров (УПП)"")");
	...
	Попытка
		...
		Результат = СоздатьДокументыПоСхеме(СтруктураОбъекта, СкладЕРП);
	...
	Исключение
		...
		мЛоггер.ерр("Субплагин: СубПлагин_док_ОтчетПроизводстваЗаСмену.
...
Функция СоздатьДокументыПоСхеме(СтруктураОбъекта, СкладЕРП)
	...
	Результат.Вставить("ЗаказНаПроизводство", Неопределено);
	...
	Результат.Вставить("ДвиженияПродукции", Неопределено);
	...
	Если МассивСсылок.Количество() >0 Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(ИмяСобытияЖР,УровеньЖурналаРегистрации.Ошибка,,,
				"Объект не загружен! Ошибка блокировки цепочки документов для "+ДокументИзУПП+".
				|Подробности: "+СообщениеОбОшибке);
			ОтменитьТранзакцию();
			
			мЛоггер.ерр("Ошибка загрузки документа (УПП): %1. 
				|Подробности: %2", ДокументИзУПП, СообщениеОбОшибке);
	...
	Попытка
		...
		ЗаказНаПроизводствоОбъект = СоздатьЗаказНаПроизводство(СтруктураОбъекта, СуществующийЗаказНаПроизводство, СкладЕРП);
		
		мТаблицаПартийЗаказа = НайтиВсеПартииЗаказа(ЗаказНаПроизводствоОбъект.Ссылка);
		
		ЭтапОбъект = СоздатьЭтап(СтруктураОбъекта, ЗаказНаПроизводствоОбъект, СуществующийЭтапСсылка);
		
		ДвПродОбъект = СоздатьДвижениеПродукцииИМатериалов(СтруктураОбъекта, 
			ЗаказНаПроизводствоОбъект, ЭтапОбъект, СуществующийДвижениеПродукцииСсылка, СкладЕРП);
		
		СкладПолучательЕРП = РегистрыСведений.ксп_ДополнительныеНастройкиИнтеграций.Настройка(
			"УПП-ЗаказНаПроизводство-СкладПолучатель", мВнешняяСистема);
		...
		Результат.Вставить("ЗаказНаПроизводство", ЗаказНаПроизводствоОбъект.Ссылка); 
		...
		Результат.Вставить("ПО", ПООбъект.Ссылка);
		Результат.Вставить("ЭтапПроизводства", ЭтапОбъект.Ссылка);
		Результат.Вставить("ДвиженияПродукции", ДвПродОбъект.Ссылка);  
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖР,УровеньЖурналаРегистрации.Ошибка,,,
			"Объект не загружен! Ошибка в процессе загрузки документа "+ДокументИзУПП+".
			|Подробности: " + ТекстОшибки);
...
Функция СоздатьЗаказНаПроизводство(СтруктураОбъекта, СуществующийЗаказСсылка, СкладЕРП)
	...
	деф = СтруктураОбъекта.definition;
	ДокументИзУПП = "Заказ на производство (УПП) № "+деф.Number+" от "+строка(деф.Date);
	...
	Если Действие = НЕ_ЗАГРУЖАТЬ Тогда
		ОтменитьТранзакцию();
		мЛоггер.дебаг("Действие = НЕ Загружать. Документ пропущен: %1", ДокументИзУПП);
    ...
    Если Действие = ПОМЕТИТЬ Тогда
        ...
        ЗафиксироватьТранзакцию();
		мЛоггер.дебаг("Действие = ПОМЕТИТЬ. Документ будет помечен на удаление: %1", ДокументИзУПП);
		Возврат ОбъектДанных;
	КонецЕсли;
	...
	Если Действие = ОТМЕНИТЬ_ПРОВЕДЕНИЕ Тогда
		...
		ЗафиксироватьТранзакцию();
		мЛоггер.дебаг("Действие = Отменить проведение. Документ: %1", ДокументИзУПП);
		Возврат ОбъектДанных;
	КонецЕсли;
	
	Если Действие = ОБНОВИТЬ Тогда
		ОбъектДанных = СуществующийЗаказСсылка.ПолучитьОбъект();
		мЛоггер.дебаг("Действие = Обновить. Документ будет обновлен: %1", ДокументИзУПП);
	ИначеЕсли Действие = СОЗДАТЬ Тогда
		...
		СсылкаНового = Документы.ЗаказНаПроизводство2_2.ПолучитьСсылку(
			Новый УникальныйИдентификатор(СтруктураОбъекта.identification.Ref));
		ОбъектДанных.УстановитьСсылкуНового(СсылкаНового);
		мЛоггер.дебаг("Действие = Создать. Документ будет создан: %1", ДокументИзУПП);
	Иначе 
		ОтменитьТранзакцию();
	...
	ОбъектДанных.Подразделение = РегистрыСведений.ксп_ДополнительныеНастройкиИнтеграций.Настройка(
		"УПП-ЗаказНаПроизводство-Подразделение-диспетчер", мВнешняяСистема);
	...
	ОбъектДанных.СпособРаспределенияЗатратНаВыходныеИзделия = РегистрыСведений.ксп_ДополнительныеНастройкиИнтеграций.Настройка(
		"УПП-ЗаказНаПроизводство-СпособРаспределенияЗатратНаВыходныеИзделия", мВнешняяСистема);
...
Функция СоздатьЭтап(СтруктураОбъекта, ЗаказНаПроизводствоОбъект, СуществующийЭтапСсылка)
	...
	ОбъектДанных = Неопределено;
	деф = СтруктураОбъекта.definition;
	ДокументИзУПП = "Этап производства (УПП) № "+деф.Number+" от "+строка(деф.Date);
	...
	Действие = ксп_ИмпортСлужебный.ДействиеСДокументом(ЭтоНовый, СуществующийЭтапСсылка, деф);
    ...
    Если Действие = ПОМЕТИТЬ Тогда
        ...
        ОбъектДанных.УстановитьПометкуУдаления(Истина);
        РегистрыСведений.ксп_ОтложенноеПроведение.УдалитьОтложенноеПроведение(СуществующийЭтапСсылка);//добавлено 2024-07-03
        ЗафиксироватьТранзакцию();
		мЛоггер.дебаг("Действие = ПОМЕТИТЬ. Документ будет помечен на удаление: %1", ДокументИзУПП);
		Возврат ОбъектДанных;
	КонецЕсли;
	...
	ОбъектДанных.ДатаВыполненияРабот = ТекущаяДатаСеанса();
	...
	ОбъектДанных.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Завершен;
	...
	Для счТовары = 0 По ЗаказНаПроизводствоОбъект.Продукция.Количество()-1 Цикл
		...
		СтрокаТЧ.ДоляСтоимостиНаЕдиницу = 10;
		СтрокаТЧ.ДоляСтоимости = СтрокаТЧ.ДоляСтоимостиНаЕдиницу * СтрокаТЧ.Количество;
	...
	ОбъектДанных.ПартияПроизводства = НайтиСоздатьПартию(
		ЗаказНаПроизводствоОбъект.Продукция[0].Номенклатура,
		ЗаказНаПроизводствоОбъект.Продукция[0].Характеристика,
		ЗаказНаПроизводствоОбъект);
...
//	Тип: Тип_значения
//
Функция СоздатьДвижениеПродукцииИМатериалов(СтруктураОбъекта, ЗаказНаПроизводствоОбъект, ЭтапОбъект, СуществующийДвижениеПродукцииСсылка, СкладЕРП)
	
	ОбъектДанных = Неопределено;
	деф = СтруктураОбъекта.definition;
	ДокументИзУПП = "Движение продукции и материалов (УПП) № "+деф.Number+" от "+строка(деф.Date);
	...
	Действие = ксп_ИмпортСлужебный.ДействиеСДокументом(ЭтоНовый, СуществующийДвижениеПродукцииСсылка, деф);
    ...
    Если Действие = ПОМЕТИТЬ Тогда
        ...
        ОбъектДанных.УстановитьПометкуУдаления(Истина);
        РегистрыСведений.ксп_ОтложенноеПроведение.УдалитьОтложенноеПроведение(СуществующийДвижениеПродукцииСсылка);//добавлено 2024-07-03
        ЗафиксироватьТранзакцию();
		мЛоггер.дебаг("Действие = ПОМЕТИТЬ. Документ будет помечен на удаление: %1", ДокументИзУПП);
		Возврат ОбъектДанных;
	КонецЕсли;
	...
	ИначеЕсли Действие = СОЗДАТЬ Тогда
		...
		СсылкаНового = Документы.ДвижениеПродукцииИМатериалов.ПолучитьСсылку(
			Новый УникальныйИдентификатор(СтруктураОбъекта.identification.Ref));
		ОбъектДанных.УстановитьСсылкуНового(СсылкаНового);
		мЛоггер.инфо("Действие = Создать. Документ будет создан: %1", ДокументИзУПП);
	Иначе 
	...
	ОбъектДанных.Получатель = СкладЕРП;
	...
	Для счТовары = 0 По ЭтапОбъект.ВыходныеИзделия.Количество()-1 Цикл
		стрк = ЭтапОбъект.ВыходныеИзделия[счТовары];
...
Функция СоздатьЗаказНаПеремещение(СтруктураОбъекта, СуществующийЗаказНаПеремещениеСсылка, СкладЕРП, 
		СкладПолучательЕРП, ЗаказНаПроизводствоОбъект)
	
	ОбъектДанных = Неопределено;
	деф = СтруктураОбъекта.definition;
	ДокументИзУПП = "Заказ на перемещение (УПП) № "+деф.Number+" от "+строка(деф.Date);
	...
	Действие = ксп_ИмпортСлужебный.ДействиеСДокументом(ЭтоНовый, СуществующийЗаказНаПеремещениеСсылка, деф);
    ...
    Если Действие = ПОМЕТИТЬ Тогда
        ...
        ОбъектДанных.УстановитьПометкуУдаления(Истина);
        РегистрыСведений.ксп_ОтложенноеПроведение.УдалитьОтложенноеПроведение(СуществующийЗаказНаПеремещениеСсылка);//добавлено 2024-07-03
        ЗафиксироватьТранзакцию();
		мЛоггер.дебаг("Действие = ПОМЕТИТЬ. Документ будет помечен на удаление: %1", ДокументИзУПП);
		Возврат ОбъектДанных;
	КонецЕсли;
	...
	ИначеЕсли Действие = СОЗДАТЬ Тогда
		...
		СсылкаНового = Документы.ЗаказНаПеремещение.ПолучитьСсылку(
			Новый УникальныйИдентификатор(СтруктураОбъекта.identification.Ref));
		ОбъектДанных.УстановитьСсылкуНового(СсылкаНового);
		мЛоггер.инфо("Действие = Создать. Документ будет создан: %1", ДокументИзУПП);
	Иначе 
...
Функция СоздатьПеремещение(СтруктураОбъекта, СуществующийПеремещениеСсылка, СкладЕРП, СкладПолучательЕРП, 
		ЗаказНаПроизводствоОбъект, ЗаказНаПеремещениеОбъект)
	
	ОбъектДанных = Неопределено;
	деф = СтруктураОбъекта.definition;
	ДокументИзУПП = "Перемещение товаров (УПП) № "+деф.Number+" от "+строка(деф.Date);
...
Функция СоздатьПО(СтруктураОбъекта, СуществующийПОСсылка, СкладПолучательЕРП, 
					ЗаказНаПеремещениеОбъект, ПеремещениеОбъект, СкладЕРП)
	
	ОбъектДанных = Неопределено;
	деф = СтруктураОбъекта.definition;
	ДокументИзУПП = "Приходный ордер на товары (УПП) № "+деф.Number+" от "+строка(деф.Date);
	...
	ИначеЕсли Действие = СОЗДАТЬ Тогда
		...
		СсылкаНового = Документы.ПриходныйОрдерНаТовары.ПолучитьСсылку(
			Новый УникальныйИдентификатор(СтруктураОбъекта.identification.Ref));
		ОбъектДанных.УстановитьСсылкуНового(СсылкаНового);
		мЛоггер.инфо("Действие = Создать. Документ будет создан: %1", ДокументИзУПП);
	Иначе 
...
Функция ТабличныеЧастиДляПроверки(ДокументСсылка = Неопределено) Экспорт
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказНаПроизводство2_2") Тогда
		
		НовСтр = ТЗ.Добавить();
		НовСтр.ИмяТЧ = "Продукция";
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		
		НовСтр = ТЗ.Добавить();
		НовСтр.ИмяТЧ = "ВыходныеИзделия";
		НовСтр.ИмяКолонки = "Номенклатура";
		НовСтр = ТЗ.Добавить();
		НовСтр.ИмяТЧ = "ВыходныеИзделия";
		НовСтр.ИмяКолонки = "Характеристика";
	...
	Если ДокументСсылка = Неопределено 
		ИЛИ ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ДвижениеПродукцииИМатериалов")
		ИЛИ ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказНаПеремещение") 
		ИЛИ ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПеремещениеТоваров") 
