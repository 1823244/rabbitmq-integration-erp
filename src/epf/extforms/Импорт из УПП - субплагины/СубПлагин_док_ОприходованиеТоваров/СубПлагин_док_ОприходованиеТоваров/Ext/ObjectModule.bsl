Перем мВидНоменклатурыСертификат ;
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Версия","2.17");
...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "", СкладЕРП = Неопределено) Экспорт
	...
	мЛоггер = Вычислить("КСП_ЛоггерСервер.getLogger(мИдВызова, ""Импорт документов из УПП: ОприходованиеТоваров (УПП)"")");
...
Функция СоздатьОбновитьДокументОприходованиеИзлишковТоваров(СтруктураОбъекта, СкладЕРП)
	...
	Попытка
		...
		УстановитьУправленческуюОрганизацию = Ложь;
		...
		Для счТовары = 0 По деф.ТЧТовары.Количество()-1 Цикл
			...
			//вычисляем только один раз, пока УстановитьУправленческуюОрганизацию = Ложь
			ЭтоПодарочныйСертификат = Ложь;
			Попытка
				ЭтоПодарочныйСертификат = ЗначениеЗаполнено(мВидНоменклатурыСертификат) 
				    И (ТипЗнч(_Номенклатура) = Тип("СправочникСсылка.Номенклатура") И ЗначениеЗаполнено(_Номенклатура.ВерсияДанных) )
					И (_Номенклатура.ВидНоменклатуры = мВидНоменклатурыСертификат);
			Исключение
			    //Сообщить(НСтр("ru = '"+ОписаниеОшибки()+"'"), СтатусСообщения.Внимание);
			КонецПопытки;
			Если ЭтоПодарочныйСертификат = Истина И УстановитьУправленческуюОрганизацию = Ложь Тогда
				УстановитьУправленческуюОрганизацию = Истина;
			КонецЕсли;
			
			Если ЭтоПодарочныйСертификат = Истина Тогда
				...
				//	ИЛИ (ТипЗнч(СтрокаТЧ.Характеристика) = Тип("СправочникСсылка.ХарактеристикиНоменклатуры") И НЕ ЗначениеЗаполнено(СтрокаТЧ.Характеристика.ВерсияДанных))
				//	Тогда
				//	
				//	СтрокаТЧ.Характеристика = ХарактеристикаСертификатаНеопределена(_Номенклатура);
				//	
				//КонецЕсли;
		...
		Если УстановитьУправленческуюОрганизацию Тогда

			ОбъектДанных.Организация =     РегистрыСведений.ксп_ДополнительныеНастройкиИнтеграций.Настройка("УправленческаяОрганизация", мВнешняяСистема);
		Иначе 
...
мВидНоменклатурыСертификат = РегистрыСведений.ксп_ДополнительныеНастройкиИнтеграций.Настройка("ВидНоменклатурыСертификатыДляОприходований", мВнешняяСистема);
