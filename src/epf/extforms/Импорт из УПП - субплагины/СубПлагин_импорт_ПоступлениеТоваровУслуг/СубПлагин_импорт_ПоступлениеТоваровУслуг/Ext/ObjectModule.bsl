...
Функция СоздатьОбновитьДокументПоступление(СтруктураОбъекта, СкладЕРП)
	...
	Попытка
		...
		кодВалюты="";
		ЕстьАтрибут = деф.Валюта.свойство("currencyCode",кодВалюты);
		Если ЕстьАтрибут Тогда
			ОбъектДанных.Валюта = ксп_ИмпортСлужебный.НайтиВалюта(кодВалюты);
...
Функция СоздатьОбновитьДокументОрдерНаОтражениеИзлишковТоваров(СтруктураОбъекта, СкладЕРП) Экспорт
	...
	id = СтруктураОбъекта.identification;
	деф = СтруктураОбъекта.definition; 
	ПредставлениеДокументаУПП = "ПоступлениеТоваровУслуг (УПП) № " + деф.Number + " от " + строка(деф.Date);
