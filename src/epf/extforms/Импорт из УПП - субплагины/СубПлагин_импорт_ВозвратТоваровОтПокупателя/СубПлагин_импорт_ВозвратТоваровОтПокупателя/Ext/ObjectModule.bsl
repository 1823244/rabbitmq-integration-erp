...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "", СкладЕРП = Неопределено) Экспорт
	...
	мЛоггер = Вычислить("КСП_ЛоггерСервер.getLogger(мИдВызова, ""Импорт документов из УПП: ВозвратТоваровОтПокупателя (УПП)"")");
...
Функция СоздатьОбновитьДокументВозвратОтПокупателя(СтруктураОбъекта, СкладЕРП)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ОбъектДанных.Соглашение) Тогда
			Если ОбъектДанных.Соглашение.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов
				ИЛИ ОбъектДанных.Соглашение.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоДоговорамНакладным	Тогда
					УзелКонтрагента = деф.Контрагент;
