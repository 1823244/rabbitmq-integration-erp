...
Функция СоздатьЗаказКлиента(СтруктураОбъекта, СуществующийЗаказСсылка, ОбъектДанных) Экспорт
	...
	КодСтроки = 1;
	ДобавитьТоварыУслуги(деф.ТЧТовары, ОбъектДанных, КодСтроки);	
	ДобавитьТоварыУслуги(деф.ТЧУслуги, ОбъектДанных, КодСтроки);	
...
//
// Параметры:
//	КодСтроки 	- число - нельзя объявлять парамет, как "Знач", он должен меняться в месте вызова!
//
		//ДобавитьТоварыУслуги(деф.ТЧТовары, ОбъектДанных)
		//ДобавитьТоварыУслуги(деф.ТЧУслуги, ОбъектДанных)

Процедура ДобавитьТоварыУслуги(деф_ТЧ, ОбъектДанных, КодСтроки)
	
	Для счТовары = 0 По деф_ТЧ.Количество()-1 Цикл
		стрк = деф_ТЧ[счТовары];
...
Функция СоздатьРеализацию(СтруктураОбъекта, РеализацияТоваровУслугСсылка, РООбъект, ЗаказКлиентаОбъект)
	...
	ОбъектДанных.Дата = деф.Date;
	ОбъектДанных.ПометкаУдаления = деф.DeletionMark;
	ОбъектДанных.Организация = ЗаказКлиентаОбъект.Организация;
	...
	ОбъектДанных.Контрагент = ЗаказКлиентаОбъект.Контрагент;
	ОбъектДанных.Партнер = ЗаказКлиентаОбъект.Партнер;
	
	ОбъектДанных.Договор = ЗаказКлиентаОбъект.Договор;
	
	ОбъектДанных.Соглашение = ЗаказКлиентаОбъект.Соглашение;
	
	//ОбъектДанных.Согласование = 
	//ОбъектДанных.Коллекция = 
	//ОбъектДанных.КаналСбыта = 
	//---
	ОбъектДанных.Валюта = ЗаказКлиентаОбъект.Валюта;
	ОбъектДанных.ВалютаВзаиморасчетов = ЗаказКлиентаОбъект.Валюта;
	ОбъектДанных.Комментарий = СтрШаблон("схема ПередачаНаКомиссию: [УПП РеализацияТоваровУслуг № %1 от %2 ]
		|СкладОтправитель: %3
		|СкладПолучатель: %4
		|Оригинальный комментарий: %5",
...
Функция СоздатьПередачуТоваровХранителю(СтруктураОбъекта, СуществующийПТХСсылка, РООбъект, ЗаказКлиентаОбъект) Экспорт
	...
	Действие = ксп_ИмпортСлужебный.ДействиеСДокументом(ЭтоНовый, СуществующийПТХСсылка, деф);
    ...
    Если Действие = ПОМЕТИТЬ Тогда
        ...
        ОбъектДанных.УстановитьПометкуУдаления(Истина);
        РегистрыСведений.ксп_ОтложенноеПроведение.УдалитьОтложенноеПроведение(СуществующийПТХСсылка);//добавлено 2024-07-03
        ЗафиксироватьТранзакцию();
		мЛоггер.дебаг("Действие = ПОМЕТИТЬ. Документ будет помечен на удаление: %1", ПредставлениеДокументаИзУПП);
		Возврат СуществующийПТХСсылка;
	КонецЕсли;
