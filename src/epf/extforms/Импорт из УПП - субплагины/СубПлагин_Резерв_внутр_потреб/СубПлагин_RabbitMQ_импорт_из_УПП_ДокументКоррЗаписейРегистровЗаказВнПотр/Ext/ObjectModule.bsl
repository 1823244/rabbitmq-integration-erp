...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "", СкладЕРП = Неопределено) Экспорт
	...
	Если Лев(деф.Комментарий,15) = "остатки для ERP" и деф.DeletionMark Тогда
		Рез = ЗагрузкаЗаказНаВнутреннееПотребление(id,деф, СтруктураОбъекта, СкладЕРП, Ложь);
	...
	ИначеЕсли Лев(деф.Комментарий,23) = "остатки розницы для ERP" и деф.DeletionMark Тогда
		Рез = ЗагрузкаЗаказНаВнутреннееПотребление(id,деф, СтруктураОбъекта, СкладЕРП, Истина);
		мЛоггер.инфо("Загружен документ УПП в документ ЕРП: "+Строка(Рез));
		возврат Рез;
	Иначе
...
Функция ЗагрузкаЗаказНаВнутреннееПотребление(id, деф, СтруктураОбъекта, СкладЕРП, ЭтоРозница, ВидОбъекта = "ЗаказНаВнутреннееПотребление")
	...
	Попытка	
		...
		Если ЭтоРозница = Истина Тогда
			ЗаполнитьДокументЗаказНаВнутреннееПотреблениеРозница(ОбъектДанных, деф, СкладЕРП);
		Иначе
...
Процедура ЗаполнитьДокументЗаказНаВнутреннееПотреблениеРозница(ОбъектДанных, деф, СкладЕРП)
	ОбъектДанных.Организация = Неопределено;;
	ОбъектДанных.Дата = деф.Date;
	ОбъектДанных.Номер = деф.Number;
	ОбъектДанных.ПометкаУдаления = Ложь;
