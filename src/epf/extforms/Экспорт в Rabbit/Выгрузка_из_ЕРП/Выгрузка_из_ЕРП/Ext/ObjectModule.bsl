...
//	Параметр1 	- Тип1 - 
//
Процедура ВыполнитьЭкспорт(ИдВызова = Неопределено, ПараметрыВыполнения = Неопределено) Экспорт

	ЗаписьЖурналаРегистрации(ИмяСобытияЖР,УровеньЖурналаРегистрации.Информация,,,
		"Запуск" );
	мИдВызова = ИдВызова;
	мИДСессии = Строка(Новый УникальныйИдентификатор);
	мЛоггер = КСП_ЛоггерСервер.СоздатьЛоггер(ИдВызова, "Экспорт в RabbitMQ");
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Метаданные.РегистрыСведений.Содержит(Метаданные.НайтиПоТипу(ТипЗнч(Объект))) Тогда
				RoutingKey = "static";
				
				ПредставлениеОбъекта = Строка(Объект);
				
				json = ксп_ЭкспортСлужебный.ВыгрузитьНаборЗаписей(Объект);
