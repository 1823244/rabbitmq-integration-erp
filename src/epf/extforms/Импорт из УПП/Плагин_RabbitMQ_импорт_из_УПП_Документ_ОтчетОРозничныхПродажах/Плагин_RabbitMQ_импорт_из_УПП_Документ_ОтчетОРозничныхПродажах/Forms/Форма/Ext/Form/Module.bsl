...
Процедура НачатьИмпорт(МассивФайлов, Доп)  Экспорт 
	...
	Для каждого ОписаниеПомещенногоФайла Из МассивФайлов Цикл  
		...
		Исключение
		    т = ОписаниеОшибки();
			КСП_ЛоггерСервер.Ошибка(ИдВызоваФорма, "файл "+строка(ОписаниеПомещенногоФайла.СсылкаНаФайл.Имя), т);
		КонецПопытки;
		
	КонецЦикла;         
	
	КСП_ЛоггерСервер.информация(ИдВызоваФорма, "", "успешно загружено файлов "+строка(сч));
...
Процедура ЗагрузитьИзJsonНаСервереИзФайла(идВызоваРодитель, Адрес)
	...
	//идВызова = КСП_ЛоггерСервер.СоздатьИдВызова(идВызоваРодитель, "Импорт ввода остатков товаров. Один документ. Интерактивный", ТекущаяДатаСеанса(),"Плагин", , Неопределено);
...
Функция СоздатьИдВызова()
	...
	идВызова = КСП_ЛоггерСервер.СоздатьИдВызова(Неопределено, "Импорт ввода остатков товаров. Интерактивный", ТекущаяДатаСеанса(),"Плагин", , Неопределено);
