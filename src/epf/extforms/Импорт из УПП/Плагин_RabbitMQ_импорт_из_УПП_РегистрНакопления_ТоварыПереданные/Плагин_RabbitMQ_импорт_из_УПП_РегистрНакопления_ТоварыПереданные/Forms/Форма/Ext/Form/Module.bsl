...
Функция ЗагрузитьИзJsonНаСервере()     
	ИдВызова = Вычислить("КСП_ЛоггерСервер.СоздатьИдВызова(, ""Импорт регистра нак ТоварыПереданные из УПП"", ТекущаяДатаСеанса(),"""", Неопределено, Неопределено)");
...
Процедура ЗагрузитьИзJson(Команда)
	...
	Выполнить("КСП_ЛоггерКлиент.ОткрытьОтчетПоЛогу(ИдВызова, Ложь, Неопределено)");
