...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	идВызова = мис_логгерСервер.СоздатьИдВызова(Неопределено, "Импорт Банковских счетов интерактивно",
	ТекущаяДата(),
...
Процедура ЗагрузитьИзJsonНаСервере()
	...
	Ссылка = обк.ЗагрузитьИзJsonНаСервере(JsonText);
...
Процедура ЗагрузитьИзJsonНаСервереИзКаталога(МассивАдресов)
	...
	_идВызова = мис_ЛоггерСервер.СоздатьИдВызова(ИдВызова, "Импорт банк. счетов. Интерактивный", ТекущаяДатаСеанса(),"Плагин", , Неопределено);
	обк=РеквизитФормыВЗначение("Объект");	
	Попытка
