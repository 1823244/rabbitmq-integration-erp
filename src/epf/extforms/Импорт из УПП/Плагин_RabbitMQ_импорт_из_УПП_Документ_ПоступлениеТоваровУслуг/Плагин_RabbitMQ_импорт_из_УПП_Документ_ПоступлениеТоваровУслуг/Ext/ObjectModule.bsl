...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	мЛоггер = мис_ЛоггерСервер.getLogger(мИдВызова, "Импорт документов из УПП: Поступление товаров");
	...
	Попытка
		...
		ПредставлениеДокументаУПП 			= "ПоступлениеТоваровУслуг №"+деф.Number+" от "+строка(деф.Date);
		...
		Если НЕ ЗНачениезаполнено(СкладУПП) Тогда
			...
			мЛоггер.ерр("Документ не загружен! Не найден склад УПП для поступления : %1", 
			ПредставлениеДокументаУПП);
		...
		Если НЕ ЗНачениезаполнено(ВидДокументаУППСсылка) Тогда
			...
			мЛоггер.ерр("Документ не загружен! Не найден Вид документа УПП ПоступлениеТоваровУслуг : %1", 
			ПредставлениеДокументаУПП);
	...
	Исключение
		...
		ТекстСообщения = "Плагин: Плагин_RabbitMQ_импорт_из_УПП_Документ_ПоступлениеТоваровУслуг . Подробности: " + ОписаниеОшибки();
		мЛоггер.ерр(ТекстСообщения);
