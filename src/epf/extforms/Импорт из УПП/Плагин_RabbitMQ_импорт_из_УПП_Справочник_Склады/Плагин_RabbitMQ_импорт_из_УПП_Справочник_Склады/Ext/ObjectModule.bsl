...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	Загрузить_Справочник_Склады(СтруктураОбъекта, jsonText, СкладУППСсылка);
	
	// здесь используется спр.Склады для мэппинга. конкретно при импорте - один-к-одному, затем пользователи поменяют если надо
...
Функция ЗаполнитьРеквизитыЭлемента_ВидыОперацийПоСкладамУПП(ОбъектДанных, СтруктураОбъекта, jsonText, СкладУППСсылка) Экспорт
	...
	уид = Новый УникальныйИдентификатор(СтруктураОбъекта.identification.Ref);
	ОбъектДанных.СкладЕРП = Справочники.Склады.ПолучитьСсылку(уид);
...
#Область ЗагрузкаВСправочник_Склады

Функция Загрузить_Справочник_Склады(
	СтруктураОбъекта, jsonText, СкладУППСсылка) Экспорт
	...
	Попытка			    
		...
		Если деф.Свойство("isFolder") И деф.isFolder = Истина Тогда
			ЗаполнитьРеквизитыГруппы_Склады(ОбъектДанных, СтруктураОбъекта, jsonText);
		Иначе 
			ЗаполнитьРеквизитыЭлемента_Склады(ОбъектДанных, СтруктураОбъекта, jsonText, СкладУППСсылка);
...
//	Тип: Тип_значения
//
Функция ПолучитьРодителя_Склады(деф)
	
	ParentRef = "";
	Если деф.parent.Свойство("Ref", ParentRef) Тогда
		Если ЗначениеЗаполнено(ParentRef) и ParentRef <> "00000000-0000-0000-0000-000000000000" Тогда
			Возврат Справочники.Склады.ПолучитьСсылку(Новый УникальныйИдентификатор(ParentRef));
		КонецЕсли;
	КонецЕсли;                                                                                       
...
// Заполняет реквизиты объекта и пишет сопутствующие данные. Должна вызываться в транзакции.
Функция ЗаполнитьРеквизитыЭлемента_Склады(ОбъектДанных, СтруктураОбъекта, jsonText, СкладУППСсылка) Экспорт
	...
	ОбъектДанных.Наименование = деф.description;
	ОбъектДанных.ПометкаУдаления = деф.DeletionMark;
	ОбъектДанных.Родитель = ПолучитьРодителя_Склады(деф);
		
	ОбъектДанных.ТипСклада = Перечисления.ТипыСкладов.ОптовыйСклад;
	ОбъектДанных.ИспользоватьОрдернуюСхемуПриОтгрузке = Истина;
	ОбъектДанных.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач = Истина;
	ОбъектДанных.ИспользоватьОрдернуюСхемуПриПоступлении = Истина;
    ОбъектДанных.ДатаНачалаОрдернойСхемыПриОтгрузке = '20240101';
	ОбъектДанных.ДатаНачалаОрдернойСхемыПриПоступлении = '20240101';
	ОбъектДанных.ДатаНачалаОрдернойСхемыПриОтраженииИзлишковНедостач = '20240101';
	ОбъектДанных.ИсточникИнформацииОЦенахДляПечати = Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоВидуЦен;
	
	Если деф.свойство("ВидСклада") Тогда
		Если ТипЗнч(деф.ВидСклада) = Тип("Структура") Тогда
			Если деф.ВидСклада.Свойство("Значение") Тогда
				Если деф.ВидСклада.Значение = "Розничный" Тогда
					ОбъектДанных.ТипСклада = Перечисления.ТипыСкладов.РозничныйМагазин;
					ОбъектДанных.ИспользоватьОрдернуюСхемуПриОтгрузке = Ложь;
					ОбъектДанных.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач = Ложь;
					ОбъектДанных.ИспользоватьОрдернуюСхемуПриПоступлении = Ложь;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	ОбъектДанных.РозничныйВидЦены=Справочники.ВидыЦен.НайтиПоНаименованию("Цена розницы Основная",Истина);
	ОбъектДанных.КонтролироватьСвободныеОстатки = Истина;	
...
Функция ЗаполнитьРеквизитыГруппы_Склады(ОбъектДанных, СтруктураОбъекта, jsonText = "") Экспорт
...
Процедура ДобавитьЗаписьВМэппингСкладов(СтруктураОбъекта)
	...
	id 	= СтруктураОбъекта.identification;
	деф = СтруктураОбъекта.definition;
	Если деф.свойство("isFolder") Тогда
