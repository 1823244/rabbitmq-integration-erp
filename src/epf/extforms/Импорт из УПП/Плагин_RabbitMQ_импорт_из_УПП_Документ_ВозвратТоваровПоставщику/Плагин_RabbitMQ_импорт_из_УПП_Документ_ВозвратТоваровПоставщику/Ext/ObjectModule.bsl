...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование","Плагин_RabbitMQ_импорт_из_УПП_Документ_ВозвратТоваровПоставщику");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация","Плагин_RabbitMQ_импорт_из_УПП_Документ_ВозвратТоваровПоставщику");
	ПараметрыРегистрации.Вставить("ВерсияБСП", "3.1.5.180");
	
	ТипКоманды = ДополнительныеОтчетыИОбработкиКлиентСервер.ТипКомандыОткрытиеФормы();
	ДобавитьКоманду(ПараметрыРегистрации.Команды, 
	"Открыть форму : Плагин_RabbitMQ_импорт_из_УПП_Документ_ВозвратТоваровПоставщику",
	"Форма_Плагин_RabbitMQ_импорт_из_УПП_Документ_ВозвратТоваровПоставщику",
	ТипКоманды, 
	Ложь) ;
...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	мЛоггер = мис_ЛоггерСервер.getLogger(мИдВызова, "Импорт документов из УПП: Возврат товаров поставщику");
	...
	Попытка
		...
		ПредставлениеДокументаУПП 			= "ВозвратТоваровПоставщику №"+деф.Number+" от "+строка(деф.Date);
		...
		ИначеЕсли 
			...
			мЛоггер.ерр("Документ не загружен! Не найден склад ХРАНЕНИЯ УПП : %1", 
			ПредставлениеДокументаУПП);
		...
		Если НЕ ЗНачениезаполнено(ВидДокументаУППСсылка) Тогда
			...
			мЛоггер.ерр("Документ не загружен! Не найден Вид документа УПП ВозвратТоваровПоставщику : %1", 
			ПредставлениеДокументаУПП);
		...
		////////////////////////////////////////////////////// ОРГАНИЗАЦИЯ
		Если деф.Организация.Свойство("Ref") Тогда
			Организация = ксп_ИмпортСлужебный.НайтиОрганизацию(деф.Организация, "upp");
			Если НЕ ЗНачениезаполнено(Организация) или (
				ТипЗнч(Организация)=Тип("СправочникСсылка.Организации") И НЕ ЗначениеЗаполнено(Организация.ВерсияДанных)
				)
				Тогда
				ВидДокумента 		= СтруктураОбъекта.type;
				Склад 				= СкладУПП;
				...
				СкладПолучатель 	= Неопределено;             
				Имя = "";
				Если деф.Организация.Свойство("Description") Тогда
					Имя = деф.Организация.Description;
				КонецЕсли;
				ТекстСообщения 		= "Не найдена Организация (по мэппингу)! " + Имя;
				ЛогикаСклад			= Неопределено;
				ЛогикаПеремещения 	= Неопределено;
				Обработчик 			= Неопределено;
				РегистрыСведений.КСП_УПП_ОшибкиИмпорта.лог(ВидДокумента, Склад, СкладОтправитель, СкладПолучатель, 
				...
				мЛоггер.ерр("Документ не загружен! Не найдена Организация (по мэппингу) : %1, Гуид организации: %2, <%3>", ПредставлениеДокументаУПП, деф.Организация.Ref, Имя);
		...
		Для каждого ВидОперацииПоСкладу Из МассивВидОперацииПоСкладу Цикл
			...
			Попытка
				Рез = ОбъектОбработчика.сетИдВызова(мИдВызова).ЗагрузитьОбъект(
				...
				Иначе
					...
					ТекстСообщения = "Субплагин вернул не структуру! Смотрите ошибки по этому документу в логе и в регистре ошибок УПП. Док: "+Строка(ПредставлениеДокументаУПП);
	...
	Исключение
		...
		ТекстСообщения = "Плагин: Плагин_RabbitMQ_импорт_из_УПП_Документ_ВозвратТоваровПоставщику . Подробности: " + ОписаниеОшибки();
		мЛоггер.ерр(ТекстСообщения);
