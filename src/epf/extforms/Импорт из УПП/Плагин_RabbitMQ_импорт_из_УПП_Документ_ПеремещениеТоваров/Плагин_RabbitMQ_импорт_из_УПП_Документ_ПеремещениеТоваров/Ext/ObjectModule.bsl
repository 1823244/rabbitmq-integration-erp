...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Версия","1.28");
...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	мЛоггер = КСП_ЛоггерСервер.getLogger(мИдВызова, "Импорт документов из УПП: Перемещение товаров");
	...
	Попытка
		...
		Попытка
			Если Вычислить("РегистрыСведений.КСП_ОграниченияИмпортаДокументовУПП.РазрешитьИмпорт(ВидДокументаУППСсылка, мСкладПолучательУПП, мСкладОтправительУПП)") = ЛОЖЬ Тогда
				
				мЛоггер.инфо("Импорт документа запрещен в регистре %1. Документ : %2", "КСП_ОграниченияИмпортаДокументовУПП", мПредставлениеДокументаУПП);
				Возврат Неопределено;
