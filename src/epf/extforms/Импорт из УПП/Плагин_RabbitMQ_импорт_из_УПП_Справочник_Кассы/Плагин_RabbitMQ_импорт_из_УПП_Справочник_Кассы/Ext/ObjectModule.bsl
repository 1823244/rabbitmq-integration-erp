...
Функция ЗагрузитьОбъект(СтруктураОбъекта, ВидНоменклатуры = Неопределено) Экспорт 
	...
	мЛоггер = КСП_ЛоггерСервер.getLogger(мИдВызова, "Импорт из УПП: Справочник.Кассы");
...
Функция ЗаполнитьРеквизиты(объектДанных, СтруктураОбъекта, ВидНоменклатуры = Неопределено) Экспорт 
	...
	Если деф.Свойство("Владелец") И деф.Владелец.Свойство("Ref") Тогда
		ОбъектДанных.Владелец = ксп_ИмпортСлужебный.НайтиОрганизацию(деф.Владелец, мВнешняяСистема);		
	КонецЕсли;
