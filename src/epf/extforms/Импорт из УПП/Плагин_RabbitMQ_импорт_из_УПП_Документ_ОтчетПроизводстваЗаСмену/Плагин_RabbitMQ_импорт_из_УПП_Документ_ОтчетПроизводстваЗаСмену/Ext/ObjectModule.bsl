...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	мЛоггер = Вычислить("мис_ЛоггерСервер.getLogger(мИдВызова, ""Импорт документов из УПП: ОтчетПроизводстваЗаСмену"")");
	...
	Попытка
		...
		Если НЕ ЗНачениезаполнено(СкладУПП) Тогда
			...
			мЛоггер.ерр("Документ не загружен! Не найден склад УПП для документа : %1", 
				ПредставлениеДокументаУПП);
			
			Возврат Неопределено;
		КонецЕсли;
		
		//он может быть пустой в источнике!
		...
		Если СкладХраненияУПП = Неопределено Тогда
			...
			мЛоггер.ерр("Не найден СкладХраненияУПП для документа : %1", ПредставлениеДокументаУПП);
		...
		ИначеЕсли
			...
			мЛоггер.ерр("Документ не загружен! Не найден склад ХРАНЕНИЯ УПП для документа : %1",
				ПредставлениеДокументаУПП);
		...
		Если НЕ ЗНачениезаполнено(ВидДокументаУППСсылка) Тогда
			...
			мЛоггер.ерр("Документ не загружен! Не найден ""Вид документа"" УПП для документа: %1", 
				ПредставлениеДокументаУПП);
		...
		Если МассивВидОперацииПоСкладу.Количество() = 0 Тогда
			...
			СкладОтправитель 	= Неопределено;
			СкладПолучатель 	= Неопределено;
			ТекстСообщения 		= "Не найден вид операции по складу в шапке и складу хранения в ТЧ Получатели! 
			|Склад: "+строка(СкладУПП)+", Склад хранения: "+строка(СкладХраненияУПП);
			ЛогикаСклад			= Неопределено;
			ЛогикаПеремещения 	= Неопределено;
			Обработчик 			= Неопределено;
			...
			мЛоггер.ерр("Документ не загружен! МассивВидОперацииПоСкладу пустой для документа: %1
				|%2", 
				ПредставлениеДокументаУПП, ТекстСообщения);
		...
		Для каждого ВидОперацииПоСкладу Из МассивВидОперацииПоСкладу Цикл
			...
			Если НЕ ЗначениеЗаполнено(ЛогикаОбработки) Тогда
				...
				мЛоггер.ерр("Документ не загружен! 
					|Не найдена Логика обработки УПП : %1, 
					|СкладУПП : %2, 
					|СкладХраненияУПП : %3, 
					|ВидОперацииПоСкладу : %4", 
					ПредставлениеДокументаУПП, СкладУПП, СкладХраненияУПП, ВидОперацииПоСкладу);
			...
			Если НЕ ЗначениеЗаполнено(СсылкаОбработчика) Тогда
				...
				мЛоггер.ерр("Документ не загружен! Не найден Субплагин : %1", ПредставлениеДокументаУПП);
			...
			Попытка
				...
				Результат = ОбъектОбработчика.сетИдВызова(мИдВызова).ЗагрузитьОбъект(
				...
				Если ТипЗнч(Результат) = Тип("Структура") Тогда
					...
					РегистрыСведений.КСП_УПП_ОшибкиИмпорта.лог(ВидДокумента, Склад, СкладОтправитель, СкладПолучатель,
						...
						деф.Number,
						деф.date,
						ОшибкаИсправлена, // ошибки исправлены
						Id.ref,
						Результат.ЗаказНаПроизводство);
					
					ТекстСообщения = "Субплагин успешно загрузил документ! Документ УПП: "+
						Строка(ПредставлениеДокументаУПП) + ", Документ ЕРП: " + Строка(Результат.ЗаказНаПроизводство);
				...
				Иначе
					...
					// Это не надо, т.к. ошибка бизнес-логики зафиксирована ранее
					ТекстСообщения = "Субплагин вернул не структуру! 
					|Смотрите ошибки по этому документу в логе и в регистре ошибок УПП. Док: "+Строка(ПредставлениеДокументаУПП);
			...
			Исключение
				...
				ТекстСообщения = "Ошибка при обработке вида операции "+Строка(ВидОперацииПоСкладу)+".
					|Переходим к следующему (если их больше одного).
					|Документ УПП: "+Строка(ПредставлениеДокументаУПП)+".
					|Подробности ошибки: "+ ОписаниеОшибки();
				ЗаписьЖурналаРегистрации(ИмяСобытияЖР, УровеньЖурналаРегистрации.Ошибка, , , ТекстСообщения);
				мЛоггер.ерр(ТекстСообщения);
	...
	Исключение
		...
		ТекстСообщения = "Плагин: Плагин_RabbitMQ_импорт_из_УПП_Документ_СписаниеТоваров . Подробности: " + ОписаниеОшибки();
		мЛоггер.ерр(ТекстСообщения);
...
Функция Найти_в_спр_КСП_ВидыОперацийПоСкладамУПП_Получатели(СкладУПП, СкладХраненияУПП)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СкладУПП", СкладУПП);
	// этот склад может быть пустым в источнике!
