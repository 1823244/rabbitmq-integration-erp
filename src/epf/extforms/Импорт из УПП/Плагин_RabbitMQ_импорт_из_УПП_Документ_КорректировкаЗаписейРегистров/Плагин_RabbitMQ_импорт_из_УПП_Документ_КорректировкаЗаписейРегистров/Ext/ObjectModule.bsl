...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	Если НЕ ЗначениеЗаполнено(мИдВызова) Тогда
		мИдВызова = КСП_ЛоггерСервер.СоздатьИдВызова(Неопределено, 
		"Импорт Корректировок записей регистров из УПП в ВводОстатковТоваров",ТекущаяДатаСеанса(), , , );
	КонецЕсли;
	
  	мЛоггер = КСП_ЛоггерСервер.getLogger(мИдВызова, "Импорт документов из УПП: Корректировка записей регистров");
    ...
    Попытка
		...
		Если СтрНайти(деф.Комментарий, "остатки для ERP") > 0 Тогда
		...
		ИначеЕсли СтрНайти(деф.Комментарий, "остатки розницы для ERP") > 0 Тогда
...
Функция ЗагрузкаОстатков(id, деф, СтруктураОбъекта, ВидОбъекта = "ВводОстатковТоваров")
	...
	Попытка			
		...
		ИначеЕсли СтрНайти(деф.Комментарий,"остатки розницы для ERP") > 0 Тогда
			...
			ЗаполнитьДокументВводОстатковТоваров(ОбъектДанных, деф, id, "РегистрНакопления_ТоварыВРознице");
		КонецЕсли;     
	...
	Если ЗначениеЗаполнено(ОбъектДанных.Ссылка) Тогда 
...
Процедура ЗаполнитьДокументВводОстатковТоваров(ОбъектДанных, деф, id, ИмяТэгаТабЧасть = "РегистрНакопления_ТоварыНаСкладах")
	...
	Если ИмяТэгаТабЧасть = "РегистрНакопления_ТоварыНаСкладах" ИЛИ ИмяТэгаТабЧасть = "" Тогда 
		Коллекция = деф.РегистрНакопления_ТоварыНаСкладах;
	ИначеЕсли ИмяТэгаТабЧасть= "РегистрНакопления_ТоварыВРознице" Тогда
		Коллекция = деф.РегистрНакопления_ТоварыВРознице;
	КонецЕсли;
	...
	Для каждого строка из Коллекция Цикл
		...
		Если Склад = Неопределено Тогда //Так НАДО!!! потому что склад в файле - в ТАбл части!!!
			// и чтобы на каждой итерации его не определеять, делаем это через флаг один раз
		...
		//				мЛоггер.варн("Запрещена загрузка по складу! Склад УПП: "+строка(СкладУПП));
		//				Возврат;
		//				
		//			КонецЕсли;	
		//			
		...
		//			
		//			мЛоггер.варн("не найден склад ЕРП! Склад УПП: "+строка(СкладУПП));
		//			
		//			РегистрыСведений.КСП_УПП_ОшибкиИмпорта.лог(
		//				"Документ.ВводОстатковТоваров",
		//				СкладУПП,
		...
		//				Неопределено);
		//				
		//				//чтобы не останавливать импорт, положим в "склад" хоть что=то
		//				склад = 1;
		//		КонецЕсли;
				уид = Новый УникальныйИдентификатор(строка.Склад.Ref);
				Склад = Справочники.Склады.ПолучитьСсылку(уид);
				ОбъектДанных.Склад = Склад;
				ОбъектДанных.Комментарий = "Документ:" + строка (id.ref) + ", склад УПП: "+строка(СкладУПП);
		//	КонецЕсли;
		КонецЕсли;
	КонецЦикла; 
	...
	мЛоггер.инфо("Количество строк в документе: "+строка(ОбъектДанных.Товары.Количество())) ;
...
Функция ХарактеристикаНеопределена(Номенклатура)
	...
	Если Рез.Пустой() Тогда		
		...
		хркт.обменДанными.Загрузка = Истина;
		хркт.Записать();
		хркт = хркт.Ссылка;
	Иначе 
		Выборка = Рез.Выбрать();
