...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование","Плагин_RabbitMQ_импорт_из_УПП_Документ_СписаниеТоваров");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация","Плагин_RabbitMQ_импорт_из_УПП_Документ_СписаниеТоваров");
	ПараметрыРегистрации.Вставить("ВерсияБСП", "3.1.5.180");
	//ПараметрыРегистрации.Вставить("ОпределитьНастройкиФормы", Ложь);
	
	ТипКоманды = ДополнительныеОтчетыИОбработкиКлиентСервер.ТипКомандыОткрытиеФормы();
	ДобавитьКоманду(ПараметрыРегистрации.Команды, 
	"Открыть форму : Плагин_RabbitMQ_импорт_из_УПП_Документ_СписаниеТоваров",
	"Форма_Плагин_RabbitMQ_импорт_из_УПП_Документ_СписаниеТоваров",
	ТипКоманды, 
	Ложь) ;
...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	мЛоггер = мис_ЛоггерСервер.getLogger(мИдВызова, "Импорт документов из УПП: СписаниеТоваров");
	...
	Попытка
		...
		ПредставлениеДокументаУПП = "СписаниеТоваров №"+деф.Number+" от "+строка(деф.Date);
		...
		Если НЕ ЗНачениезаполнено(СкладУПП) Тогда
			...
			мЛоггер.ерр("Документ не загружен! Не найден склад УПП для Списания : %1", 
		...
		ИначеЕсли 
			...
			мЛоггер.ерр("Документ не загружен! Не найден склад ХРАНЕНИЯ УПП для Списания : %1", 
			ПредставлениеДокументаУПП);
		...
		Если НЕ ЗНачениезаполнено(ВидДокументаУППСсылка) Тогда
			...
			мЛоггер.ерр("Документ не загружен! Не найден Вид документа УПП Списание : %1", 
			ПредставлениеДокументаУПП);
		...
		Для каждого ВидОперацииПоСкладу Из МассивВидОперацииПоСкладу Цикл  
			...
			Если ВидОперацииПоСкладу.Активность Тогда 
				// всё ок, продолжаем
			Иначе
				...
				СкладОтправитель 	= Неопределено;
				СкладПолучатель 	= Неопределено;
				ТекстСообщения 		= "Для ВидыОперацийПоСкладамУПП: " + ВидОперацииПоСкладу + " не установлена Активность!";
				ЛогикаСклад			= Неопределено;
				ЛогикаПеремещения 	= Неопределено;
				Обработчик 			= Неопределено;
				РегистрыСведений.КСП_УПП_ОшибкиИмпорта.лог(ВидДокумента, Склад, СкладОтправитель, СкладПолучатель, 
			...
			Попытка
				...
				Если ТипЗнч(Рез) = Тип("Структура") Тогда
					...
					Id.ref,
					Рез.СписаниеНедостачТоваров);
					
					ТекстСообщение = "Субплагин успешно загрузил документ! Документ УПП: "+Строка(ПредставлениеДокументаУПП)
						+", Документ ЕРП: " + строка(Рез.СписаниеНедостачТоваров);
					ЗаписьЖурналаРегистрации(ИмяСобытияЖР, УровеньЖурналаРегистрации.Информация,,,ТекстСообщение);
					мЛоггер.инфо(ТекстСообщение);
					
				Иначе 
					
					// Это не надо, т.к. ошибка бизнес-логики зафиксирована ранее
					т = "Субплагин вернул не структуру! Смотрите ошибки по этому документу в логе и в регистре ошибок УПП. Док: "+Строка(ПредставлениеДокументаУПП);
			...
			Исключение
				т="Ошибка при обработке вида операции "+Строка(ВидОперацииПоСкладу)+". Переходим к следующему (если их больше одного). Документ УПП: "+Строка(ПредставлениеДокументаУПП)+". Подробности ошибки: "+ ОписаниеОшибки();
				ЗаписьЖурналаРегистрации(ИмяСобытияЖР, УровеньЖурналаРегистрации.Ошибка,,,т);
				мЛоггер.ерр(т);   
	...
	Исключение
		т = "Плагин: Плагин_RabbitMQ_импорт_из_УПП_Документ_СписаниеТоваров . Подробности: " + ОписаниеОшибки();
		мЛоггер.ерр(т);
