...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Версия","1.5.поток");
	//ПараметрыРегистрации.Вставить("Назначение", Новый Массив);
	ПараметрыРегистрации.Вставить("Наименование","Плагин_RabbitMQ_импорт_из_УПП_Справочник_Артикул");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация","Плагин_RabbitMQ_импорт_из_УПП_Справочник_Артикул");
	...
	ДобавитьКоманду(ПараметрыРегистрации.Команды, 
		"Открыть форму : Плагин_RabbitMQ_импорт_из_УПП_Справочник_Артикул",
		"Форма_Плагин_RabbitMQ_импорт_из_УПП_Справочник_Артикул",
		ТипКоманды, 
		Ложь) ;
...
Функция ЗагрузитьОбъект(СтруктураОбъекта, ВидНоменклатуры = Неопределено) Экспорт 
	...
	мЛоггер = мис_ЛоггерСервер.getLogger(мИдВызова, "Импорт из УПП: Справочник.Артикул");
    ...
    Попытка
		...
		Если НЕ НРег(СтруктураОбъекта.type) = "справочник.артикул" Тогда  
			млоггер.варн("Пропущено! В type не Справочник.Артикул");
			Возврат Неопределено;
		КонецЕсли;
...
Функция ЗаполнитьРеквизиты(объектДанных, СтруктураОбъекта, ВидНоменклатуры = Неопределено) Экспорт 
	...
	//ОбъектДанных.ВидПродукции                 = Номенклатура.ВидПродукции; //СправочникСсылка.КСП_ВидыПродукции
	...
	Если деф.цвет.свойство("Наименование") Тогда
		ОбъектДанных.Цвет 						= Справочники.КСП_Цвета.НайтиПоНаименованию(деф.цвет.Наименование,Истина);
	КонецЕсли;
