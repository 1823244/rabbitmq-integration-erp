...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование","Импорт из 1С:УПП (Реконнект в цикле)");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация","Импорт из 1С:УПП (Реконнект в цикле)");
...
//
Процедура ВыполнитьИмпортДокументов(Знач ИдВызова = Неопределено, СерверRabbitMQ = Неопределено, ОчередьRabbitMQ = Неопределено) Экспорт
	
	мИДСессии = Строка(Новый УникальныйИдентификатор);
	
	мИдВызова = ИдВызова;
	мЛоггер = мис_ЛоггерСервер.getLogger(ИдВызова, "Импорт документов из УПП");
	
	мЛоггер.инфо("Запуск. Имя сервера: %1", СерверRabbitMQ);
	
	Если ЗначениеЗаполнено(ОчередьRabbitMQ) Тогда
		Очередь = ОчередьRabbitMQ;
		мЛоггер.инфо("Запуск. Имя очереди: %1 (передана из параметра формы)", Очередь);
	Иначе 
		Очередь = "to-erp-from-upp-doc";
		мЛоггер.инфо("Запуск. Имя очереди: %1 (очередь по-умолчанию)", Очередь);
	КонецЕсли;
	...
	ЗагрузитьДанныеИзОчереди(СерверRabbitMQ, Очередь);
	
	мЛоггер.инфо("Завершение");
		
КонецПроцедуры

Процедура ВыполнитьИмпортНСИ(Знач ИдВызова = Неопределено, СерверRabbitMQ = Неопределено, ОчередьRabbitMQ = Неопределено) Экспорт
	
	мИДСессии = Строка(Новый УникальныйИдентификатор);
	
	мЛоггер = мис_ЛоггерСервер.getLogger(ИдВызова, "Импорт НСИ из УПП");
	...
	Исключение                               
	    ...
	    мЛоггер.ерр("ВыполнитьИмпортНСИ:: "+т);
	КонецПопытки;
...
Функция Подключиться()
	...
	selectSize = РегистрыСведений.ксп_ДополнительныеНастройкиИнтеграций.Настройка("RabbitMQРазмерПакетаЧтения", "");
	Если НЕ ЗначениеЗаполнено(selectSize) Тогда
	...
	Исключение
		...
		"Ошибка получения сообщений: мКлиент.BasicConsume(). Сессия ИД "+мИДСессии+". Подробности: "+т
		+Символы.ПС+СтрокаПодключенияДляЖР);
...
//
Процедура ЗагрузитьДанныеИзОчереди(СерверRabbitMQ, пИмяОчереди, Знач пКлиент = Неопределено) Экспорт
	...
	Попытка
		...
		Если пКлиент = Неопределено Тогда
			Выполнить("мКлиент = PinkRabbit.ПолучитьКлиента( СерверRabbitMQ );");
...
Функция ВнутреннийЦикл() Экспорт
	...
	Пока мКлиент.BasicConsumeMessage(consumerId, ПолучаемоеСообщение, мТегСообщения, 3000) Цикл
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(ИмяСобытияЖР,УровеньЖурналаРегистрации.Предупреждение,,,"Успешно установлен ИдВызова в плагин "+Строка(Плагин));
		Исключение
		...
		мЛоггер.дебаг("Версия плагина "+строка(Плагин)+" = "+Строка(
			Плагин.СведенияОВнешнейОбработке().Версия
		));
		Попытка	
