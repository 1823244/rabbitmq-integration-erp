...
Процедура ВыполнитьИмпортНСИНаСервере()
	...
	мИдВызова = Вычислить("КСП_ЛоггерСервер.СоздатьИдВызова(, ""Импорт НСИ из УПП"", ТекущаяДатаСеанса(),"""", Неопределено, Неопределено)");
...
Процедура ВыполнитьИмпортНСИ(Команда)
	...
	Выполнить("КСП_ЛоггерКлиент.ОткрытьОтчетПоЛогу(мИдВызова, Ложь, Неопределено)");
КонецПроцедуры

&НаСервере
Процедура ВыполнитьИмпортДокументовНаСервере()

	мИдВызова = Вычислить("КСП_ЛоггерСервер.СоздатьИдВызова(, ""Импорт Документов из УПП"", ТекущаяДатаСеанса(),"""", Неопределено, Неопределено)");
