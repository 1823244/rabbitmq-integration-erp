...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	мЛоггер = КСП_ЛоггерСервер.getLogger(мИдВызова, "Импорт документов из УПП: Комплектация номенклатуры");
    ...
    Попытка
		...
		Если НЕ СтрНайти(СтруктураОбъекта.definition.Комментарий,"ПереносПоследнихЦенВЕРП") = 0 Тогда 
			
			СтруктураОбъекта.definition.DeletionMark = Ложь;
			СтруктураОбъекта.definition.isposted = Истина;
...
Функция СоздатьОбновитьДокумент(СтруктураОбъекта) Экспорт
	...
	Исключение
		...
		Если ТранзакцияАктивна() Тогда
			ОтменитьТранзакцию();
		КонецЕсли;
		ошибка = "Объект не загружен! Ошибка в процессе загрузки документа " + ДокументИзУПП + ". Подробности: " + т;
...
Функция ЗаполнитьРеквизиты(СтруктураОбъекта, ОбъектДанных)
	...
	Если деф.Ответственный.Свойство("Ref") Тогда                                  
		уид = новый УникальныйИдентификатор(деф.Ответственный.Ref);
		ОбъектДанных.Ответственный = Справочники.Пользователи.ПолучитьСсылку(Уид);
	КонецЕсли;
	...
	Если деф.ТЧТовары.Количество()> 0 Тогда
		...
		Для счТовары = 0 По деф.ТЧТовары.Количество()-1 Цикл
			...
			Если СобиратьНенайденнуюНоменклатуру и ТэгНоменклатуры.Свойство("ref") Тогда
				Если ТипЗнч(_Номенклатура) = Тип("СправочникСсылка.Номенклатура") И НЕ ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(_Номенклатура,"ВерсияДанных")) Тогда
			...
			Характеристика = ксп_ИмпортСлужебный.НайтиХарактеристику(стрк.ХарактеристикаНоменклатуры); 
			
			Если ТипЗнч(Характеристика) = Тип("СправочникСсылка.ХарактеристикиНоменклатуры") И ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Характеристика,"ВерсияДанных")) Тогда 
				
				СтрокаТЧ.ХарактеристикаЦО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Характеристика,"ХарактеристикаНоменклатурыДляЦенообразования");
	...
	ДополнитьХарактеристикамиЦО(ОбъектДанных.Товары2_5);
...
Процедура ДополнитьХарактеристикамиЦО(ТабличнаяЧасть)
	...
	Запрос.Текст = 
		...
		|	Таблица.ХарактеристикаЦО КАК ХарактерситикаЦО,
		...
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТТабличнаяЧасть.ХарактерситикаЦО) КАК ХарактерситикаЦО
		|ПОМЕСТИТЬ ВТНоменклатураДополнения
		|ИЗ
		|	ВТТабличнаяЧасть КАК ВТТабличнаяЧасть
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТТабличнаяЧасть.Номенклатура
		|
		|ИМЕЮЩИЕ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТТабличнаяЧасть.ХарактерситикаЦО) = 1
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|			ИЗ
		|				ВТНоменклатураДополнения КАК Таблица)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТХарактеристикиЦО.Владелец КАК Владелец,
		|	ВТХарактеристикиЦО.ХарактеристикаНоменклатурыДляЦенообразования КАК ХарактеристикаНоменклатурыДляЦенообразования,
		|	ВТТабличнаяЧасть.ВидЦены КАК ВидЦены,
		|	ВТТабличнаяЧасть.Цена КАК Цена,
		|	ВТТабличнаяЧасть.Валюта КАК Валюта
		|ПОМЕСТИТЬ ВТИтогПоХарактеристикам
		|ИЗ
		|	ВТХарактеристикиЦО КАК ВТХарактеристикиЦО
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТТабличнаяЧасть КАК ВТТабличнаяЧасть
		|		ПО ВТХарактеристикиЦО.Владелец = ВТТабличнаяЧасть.Номенклатура
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТИтогПоХарактеристикам.ВидЦены КАК ВидЦены,
		|	ВТИтогПоХарактеристикам.Цена КАК Цена,
		|	ВТИтогПоХарактеристикам.Валюта КАК Валюта,
		|	ВТИтогПоХарактеристикам.Владелец КАК Номенклатура,
		|	ВТИтогПоХарактеристикам.ХарактеристикаНоменклатурыДляЦенообразования КАК ХарактеристикаЦО
		|ИЗ
		|	ВТИтогПоХарактеристикам КАК ВТИтогПоХарактеристикам
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
		|	ВТТабличнаяЧасть.ВидЦены,
		|	ВТТабличнаяЧасть.Цена,
		|	ВТТабличнаяЧасть.Валюта,
		|	ВТТабличнаяЧасть.Номенклатура,
		|	ВТТабличнаяЧасть.ХарактерситикаЦО
		|ИЗ
		|	ВТТабличнаяЧасть КАК ВТТабличнаяЧасть";
	
	Запрос.УстановитьПараметр("Таблица",ТабличнаяЧасть.Выгрузить());
	
	РезультатЗапроса = Запрос.Выполнить().Выгрузить();
	
	ТабличнаяЧасть.Загрузить(РезультатЗапроса);
