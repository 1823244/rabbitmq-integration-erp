...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	мЛоггер = Вычислить("мис_ЛоггерСервер.getLogger(мИдВызова, ""Импорт документов из УПП: Комплектация номенклатуры"")");
	...
	Попытка
		...
		ИначеЕсли 
			...
			мЛоггер.ерр("Документ не загружен! Не найден склад ХРАНЕНИЯ УПП для Комплектации номенклатуры : %1",
			ПредставлениеДокументаУПП);
		...
		Если МассивВидОперацииПоСкладу.Количество() = 0 Тогда
			...
			СкладОтправитель 	= Неопределено;
			СкладПолучатель 	= Неопределено;
			ТекстСообщения 		= "Не найден в ТЧ Получатели вид операции по складу! Склад: "+строка(СкладУПП)+", Склад хранения: "+строка(СкладХраненияУПП);
			ЛогикаСклад			= Неопределено;
			ЛогикаПеремещения 	= Неопределено;
			Обработчик 			= Неопределено;
			РегистрыСведений.КСП_УПП_ОшибкиИмпорта.лог(ВидДокумента, Склад, СкладОтправитель, СкладПолучатель, 
		...
		Для каждого ВидОперацииПоСкладу Из МассивВидОперацииПоСкладу Цикл
			...
			Попытка
				...
				Если ТипЗнч(Рез) = Тип("Структура") Тогда
					...
					ТекстОшибки = "Субплагин успешно загрузил документ! Документ УПП: "+Строка(ПредставлениеДокументаУПП)+", Документ ЕРП: "
					+строка(Рез.СборкаТоваров);
					ЗаписьЖурналаРегистрации(ИмяСобытияЖР, УровеньЖурналаРегистрации.Информация,,,ТекстОшибки);
					мЛоггер.инфо(ТекстОшибки);
				Иначе 
					
					// Это не надо, т.к. ошибка бизнес-логики зафиксирована ранее
					ТекстОшибки = "Субплагин вернул не структуру! Смотрите ошибки по этому документу в логе и в регистре ошибок УПП. Док: "+Строка(ПредставлениеДокументаУПП);
			...
			Исключение
				ТекстОшибки="Ошибка при обработке вида операции "+Строка(ВидОперацииПоСкладу)+". Переходим к следующему (если их больше одного). Документ УПП: "+Строка(ПредставлениеДокументаУПП)+". Подробности ошибки: "+ ОписаниеОшибки();
				ЗаписьЖурналаРегистрации(ИмяСобытияЖР, УровеньЖурналаРегистрации.Ошибка,,,ТекстОшибки);
				мЛоггер.ерр(ТекстОшибки);
	...
	Исключение
		ТекстОшибки = "Плагин: Импорт Документ.КомплектацияНоменклатуры. Подробности: " + ОписаниеОшибки();
		мЛоггер.ерр(ТекстОшибки);
