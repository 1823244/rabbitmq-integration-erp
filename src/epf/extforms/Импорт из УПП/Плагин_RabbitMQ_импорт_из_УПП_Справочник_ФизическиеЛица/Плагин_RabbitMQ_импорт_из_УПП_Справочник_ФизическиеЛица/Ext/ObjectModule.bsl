...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Версия","1.6.Асинх.Поток");
...
Функция ЗаполнитьРеквизиты(объектДанных, СтруктураОбъекта, ВидНоменклатуры = Неопределено) Экспорт 
	...
	ОбъектДанных.ФИО = сокрлп(def.description);
	ParentRef = "";
	...
	Пол = "";
	Если def.Пол.Свойство("Значение", Пол) Тогда
		Если Пол = "Женский" Тогда
			ОбъектДанных.Пол = Перечисления.ПолФизическогоЛица.Женский;
		ИначеЕсли Пол = "Мужской" Тогда
			ОбъектДанных.Пол = Перечисления.ПолФизическогоЛица.Мужской;
		КонецЕсли;
	КонецЕсли;
