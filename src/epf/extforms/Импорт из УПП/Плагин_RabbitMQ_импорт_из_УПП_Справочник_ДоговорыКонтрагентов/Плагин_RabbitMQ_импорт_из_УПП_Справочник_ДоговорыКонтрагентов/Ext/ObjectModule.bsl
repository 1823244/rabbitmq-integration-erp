...
Функция ЗагрузитьОбъект(СтруктураОбъекта) Экспорт
	мЛоггер = Вычислить("КСП_ЛоггерСервер.getLogger(мИдВызова, ""Импорт из УПП: справочник Договоры контрагентов"")");
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ОбъектДанных.Контрагент) Тогда
			ОбъектДанных.Партнер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектДанных.Контрагент, "Партнер");
		КонецЕсли;
		
		ОбъектДанных.Организация = ПолучитьСсылкуСправочникаПоДаннымID(def.Организация, "Организации");
		
		Если def.Валюта.Свойство("currencyCode") Тогда
			ОбъектДанных.ВалютаВзаиморасчетов = Справочники.Валюты.НайтиПоКоду(def.Валюта.currencyCode);
		КонецЕсли;
		...
		Если def.Свойство("ТипЦен") Тогда
