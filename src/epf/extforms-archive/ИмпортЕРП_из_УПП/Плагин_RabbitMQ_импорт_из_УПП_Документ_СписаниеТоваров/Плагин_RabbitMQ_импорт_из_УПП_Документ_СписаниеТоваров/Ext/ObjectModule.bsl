		...
		Для каждого ВидОперацииПоСкладу Из МассивВидОперацииПоСкладу Цикл
            ...
            Попытка        
				...
				Если ТипЗнч(Рез) = Тип("Структура") Тогда
					...
					т = "Субплагин успешно загрузил документ! Документ УПП: "+Строка(ПредставлениеДокументаУПП)+", Документ ЕРП: "
						+строка(Рез.Списание);
					ЗаписьЖурналаРегистрации(ИмяСобытияЖР, УровеньЖурналаРегистрации.Информация,,,т);
					мЛоггер.инфо(т);   
				Иначе 
