...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование","Импорт из 1С:БазаПоказов");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация","Импорт из 1С:БазаПоказов");
	...
	ДобавитьКоманду(ПараметрыРегистрации.Команды, "Открыть форму : Импорт из 1С:БазаПоказов","ЗагрузитьДанныеИз1СБазаПоказовИнтерактивно",ТипКоманды, Ложь) ;
	
	ТипКоманды = ДополнительныеОтчетыИОбработкиКлиентСервер.ТипКомандыВызовСерверногоМетода();
	ДобавитьКоманду(ПараметрыРегистрации.Команды, "Выполнить импорт НСИ из 1С:БазаПоказов","ЗагрузитьНСИИз1СБазаПоказов",ТипКоманды, Ложь) ;
	
	ТипКоманды = ДополнительныеОтчетыИОбработкиКлиентСервер.ТипКомандыВызовСерверногоМетода();
	ДобавитьКоманду(ПараметрыРегистрации.Команды, "Выполнить импорт документов из 1С:БазаПоказов","ЗагрузитьДокументыИз1СБазаПоказов",ТипКоманды, Ложь) ;
...
Процедура ВыполнитьКоманду(ИмяКоманды, ПараметрыВыполнения) Экспорт
	...
	Если ИмяКоманды = "ЗагрузитьДокументыИз1СБазаПоказов" Тогда
		ВыполнитьИмпортДокументов();
	ИначеЕсли ИмяКоманды = "ЗагрузитьНСИИз1СБазаПоказов" Тогда
		ВыполнитьИмпортНСИ();
	КонецЕсли;
...
Процедура ВыполнитьИмпортДокументов(Знач ИдВызова = Неопределено) Экспорт
	...
	мИдВызова = ИдВызова;
	мЛоггер = мис_ЛоггерСервер.getLogger(ИдВызова, "Импорт документов из БазаПоказов");
	
    Очередь = "to-erp-from-bazap-doc";
...
Процедура ВыполнитьИмпортНСИ(Знач ИдВызова = Неопределено) Экспорт
	...
	мЛоггер = мис_ЛоггерСервер.getLogger(ИдВызова, "Импорт НСИ из БазаПоказов");
	
    Очередь = "to-erp-from-bazap-static";
...
Функция НайтиПлагиныВДопОбработках()
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	спр.ИмяОбъекта ПОДОБНО ""Плагин_RabbitMQ_импорт_из_БП%""
