...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ДобавитьКоманду(ПараметрыРегистрации.Команды, 
		...
		"Форма_Плагин_Плагин_RabbitMQ_экспорт_Справочник_Кассы",
		ТипКоманды, 
		Ложь) ;
...
Функция ВыгрузитьОбъект(ДанныеСсылка) Экспорт
	...
	definition.Вставить("ВалютаДенежныхСредств", ксп_ЭкспортСлужебный.СоздатьУзелIdentification(Обк.ВалютаДенежныхСредств));
	definition.Вставить("ГруппаФинансовогоУчета", ксп_ЭкспортСлужебный.СоздатьУзелIdentification(Обк.ГруппаФинансовогоУчета));
	definition.Вставить("КассоваяКнига", ксп_ЭкспортСлужебный.СоздатьУзелIdentification(Обк.КассоваяКнига));
	definition.Вставить("НаправлениеДеятельности", ксп_ЭкспортСлужебный.СоздатьУзелIdentification(Обк.НаправлениеДеятельности));
	definition.Вставить("ОперационнаяКасса", Обк.ОперационнаяКасса);
	definition.Вставить("Подразделение", ксп_ЭкспортСлужебный.СоздатьУзелIdentification(Обк.Подразделение));
	definition.Вставить("РазрешитьПлатежиБезУказанияЗаявок", Обк.РазрешитьПлатежиБезУказанияЗаявок);
	definition.Вставить("СрокИнкассации", Обк.СрокИнкассации);
	definition.Вставить("ФизическоеЛицо", ксп_ЭкспортСлужебный.СоздатьУзелIdentification(Обк.ФизическоеЛицо));
	definition.Вставить("ЭтоКассаОбособленногоПодразделения", Обк.ЭтоКассаОбособленногоПодразделения);

	//------------------------------------------------------     ТЧ ПолучателиПлатежейПриПеремещенииДС

	ТЧПолучателиПлатежейПриПеремещенииДС = Новый Массив;

	Для сч = 0 По обк.ПолучателиПлатежейПриПеремещенииДС.Количество()-1 Цикл

		стрк = обк.ПолучателиПлатежейПриПеремещенииДС[сч];

		НовСтр = Новый Структура;

		НовСтр.Вставить("ПолучательПлатежа", ксп_ЭкспортСлужебный.СоздатьУзелIdentification(стрк.ПолучательПлатежа));
		ТЧПолучателиПлатежейПриПеремещенииДС.Добавить(НовСтр);

	КонецЦикла;

	definition.Вставить("ТЧПолучателиПлатежейПриПеремещенииДС", ТЧПолучателиПлатежейПриПеремещенииДС);
