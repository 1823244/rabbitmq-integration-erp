Перем ВСЕГО_ОБНОВЛЕНО;
Перем ВСЕГО_ПРОПУЩЕНО;
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование","Импорт_Из_УПП_AdoConnection_Номенклатура");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация","Импорт_Из_УПП_AdoConnection_Номенклатура");
	...
	ДобавитьКоманду(ПараметрыРегистрации.Команды, 
		"Открыть форму : Импорт_Из_УПП_AdoConnection_Номенклатура",
		"Форма_Импорт_Из_УПП_AdoConnection_Номенклатура",
		ТипКоманды, 
		Ложь) ;
...
Функция ТЗрекордсет() Экспорт
	...
	ТЗРекордсет.Колонки.Добавить("Ref", ТипСтрока);  	//0 гуид
	ТЗРекордсет.Колонки.Добавить("IsFolder");			//1 булево
	ТЗРекордсет.Колонки.Добавить("Parent", ТипСтрока);  //2 гуид
	ТЗРекордсет.Колонки.Добавить("VidNomenklatury", ТипСтрока);  //3 не нужен пока
	ТЗРекордсет.Колонки.Добавить("VidVosproizv", ТипСтрока);  	//4 строка
	ТЗРекордсет.Колонки.Добавить("TNVED", ТипСтрока);  			//5 строка, наим
	ТЗРекордсет.Колонки.Добавить("Description", ТипСтрока);  	//6 строка
	ТЗРекордсет.Колонки.Добавить("TNVED_Kod", ТипСтрока);  		//7 строка, код
	ТЗРекордсет.Колонки.Добавить("Artikul", ТипСтрока);         //8 строка
	ТЗРекордсет.Колонки.Добавить("StavkaNDS", ТипСтрока);       //9 гуид
	ТЗРекордсет.Колонки.Добавить("Capsula", ТипСтрока);         //10 гуид
	ТЗРекордсет.Колонки.Добавить("Season", ТипСтрока);          //11 гуид
	ТЗРекордсет.Колонки.Добавить("NaimenovaniePolnoe", ТипСтрока);    //12 строка
	ТЗРекордсет.Колонки.Добавить("Collection", ТипСтрока);            //13 гуид
	ТЗРекордсет.Колонки.Добавить("PeriodVyveski", ТипСтрока);         //14 гуид
	ТЗРекордсет.Колонки.Добавить("Proizvoditel", ТипСтрока);          //15 строка , наим
	ТЗРекордсет.Колонки.Добавить("Project", ТипСтрока);               //16 гуид
...
//	Драйвер - строка - значения см. в методе ConnectionString()
//
Процедура ЗагрузитьВсеНаСервере(Знач ИдВызова,ИмяСервера,ИмяБазыДанных,
	ИмяПользователя,Пароль,Знач РазмерПакета, Обновлять,МестоЗапуска,Драйвер) Экспорт
	
	ВремяНач = ТекущаяУниверсальнаяДатаВМиллисекундах();
	
	ИдВызова = мис_логгерСервер.СоздатьИдВызова(ИдВызова, "ЗагрузитьВсеНаСервере", ТекущаяДата(),"", Неопределено, Неопределено);
	мЛоггер = мис_логгерСервер.getLogger(ИдВызова);
	млоггер.инфо("ЗагрузитьВсеНаСервере запуск.");
	...
	ИначеЕсли МестоЗапуска = 1 Тогда
		...
		|	source._IDRRef AS Ref,
		|	source._Folder AS IsFolder,
		|	source._ParentIDRRef AS Parent,
		|	source._Fld1431RRef AS VidNomenklatury,
		|	source._Fld1430RRef AS VidVosproizv,
		|	KodTNVED._Description AS TNVED,
		|	source._Description AS Description,
		|	KodTNVED._Code AS TNVED_Kod,
		
		|	_Fld1421 AS Artikul,
		|	_Fld1444RRef AS StavkaNDS,
		|	_Fld23809RRef AS Capsula,
		|	_Fld23821RRef AS Season,
		|	_Fld1437 AS NaimenovaniePolnoe,
		|	_Fld23810RRef AS Collection,
		|	_Fld23816RRef AS PeriodVyveski,
		|	производители._Description AS Proizvoditel,
		|	Collections._Fld18871RRef AS Project
       	
		|	
		|FROM
		|	_Reference95 as source 
		|	left join _Reference32613 as KodTNVED  on source._Fld32614RRef = KodTNVED._IDRRef
		|	left join _Reference18333 as COllections on source._Fld23810RRef = COllections._IDRRef
		|	left join производители as Manufacturer on source._Fld23819RRef = производители._IDRRef
		|
		|
		|
		|";
	...
	ПараметрыМетодаФонЗадания.Вставить("ИмяВнешнейОбработки",		"Импорт_Из_УПП_AdoConnection_Номенклатура");
	...
	//сообщить("Режим отладки = "+строка(ОбщегоНазначения.РежимОтладки() ));
	...
	//		.сетОписаниеФоновогоЗадания("Параллельная загрузка Номенклатуры из УПП")
	//		.сетИмяМетодаФоновогоЗадания(ИмяМетодаФонЗадания)
	//		.сетРазмерПакета(РазмерПакета)
	//		.сетШаблонТЗРекордсет(ЗначениеВСтрокуВнутр(ТЗРекордсет()))
	...
	//Иначе 
		...
		while not recordSet.eof do
			...
			while not recordSet.eof И КолВПакете < РазмерПакета do
				...
				НовСтр.IsFolder 		= recordSet.Fields.Item(1).value;
				
				НовСтр.VidVosproizv 	= ВидВоспроизводства(  recordSet.Fields.Item(4).value );
				НовСтр.TNVED	 		= recordSet.Fields.Item(5).value;
				НовСтр.Description 		= recordSet.Fields.Item(6).value;
				НовСтр.Ref 				= ПолучитьГУИДстрокой(recordSet.Fields.Item(0).value);
				НовСтр.Parent 			= ПолучитьГУИДстрокой(recordSet.Fields.Item(2).value);
				НовСтр.TNVED_Kod 		= recordSet.Fields.Item(7).value;
				
				//мЛоггер.инфо("ГУИД из CAST(): "+recordSet.Fields.Item(0).value+", ГУИД из RAW: "+ггг+", Binary: "+строка(бин)+", Наим: "+строка(НовСтр.Description));
				
				НовСтр.Artikul 			= recordSet.Fields.Item(8).value;
				НовСтр.StavkaNDS 		= ПолучитьГУИДстрокой(recordSet.Fields.Item(9).value);
				НовСтр.Capsula 			= ПолучитьГУИДстрокой(recordSet.Fields.Item(10).value);
				НовСтр.Season 			= ПолучитьГУИДстрокой(recordSet.Fields.Item(11).value);
				НовСтр.NaimenovaniePolnoe 			= recordSet.Fields.Item(12).value;
				НовСтр.Collection 		= ПолучитьГУИДстрокой(recordSet.Fields.Item(13).value);
				НовСтр.PeriodVyveski 	= ПолучитьГУИДстрокой(recordSet.Fields.Item(14).value);
				НовСтр.Proizvoditel 	= recordSet.Fields.Item(15).value; //строка, наим
				НовСтр.Project 			= ПолучитьГУИДстрокой(recordSet.Fields.Item(16).value);
				
				
				

				//млоггер.инфо("Обход рекордсета. Номенклатура гуид = " + ном);
				//млоггер.инфо("ТЗРекордсет. Номенклатура гуид  (колонка goods) = " + НовСтр.goods);
			...
			ДопПараметры = Новый структура("Обновлять", Обновлять);
			ОбработатьПакет_Фон(ИдВызова, ТЗРекордсет, СчЗаданий, ДопПараметры);
	...
	млоггер.инфо("ЗагрузитьВсеНаСервере ЗАВЕРШЕНИЕ");
	
	Время = ТекущаяУниверсальнаяДатаВМиллисекундах() - ВремяНач;
		млоггер.инфо("Время = %1 мс. "+
		"Загружено номенклатуры %3 из %2. Пропущено номенклатуры %4", 
			строка(Время), СчетчикВсего, ВСЕГО_ОБНОВЛЕНО , ВСЕГО_ПРОПУЩЕНО);
...
Процедура ОбработатьПакет_Фон(Знач ИдВызова, ТЗРекордсет, НомерЗадания, ДопПараметры) Экспорт	
	...
	ИдВызова = мис_логгерСервер.СоздатьИдВызова(ИдВызова, "ОбработатьПакет_Фон. Номер задания = "+Строка(НомерЗадания), ТекущаяДата(),"", Неопределено, Неопределено);
	мЛоггер = мис_логгерСервер.getLogger(ИдВызова);
	...
	Попытка
		...
		Для каждого стрк Из ТЗРекордсет Цикл
			...
			гуидСтрока = стрк.Ref;
			мЛоггер.дебаг("гуидСтрока = "+гуидСтрока);
			
			УИД = Новый УникальныйИдентификатор(гуидСтрока);
			
			Номенклатура = Справочники.Номенклатура.ПолучитьСсылку(УИД);            
			
			НаимДляЛога = СтрЗаменить(стрк.description, "%", "_");
			
			Если НЕ ЗначениеЗаполнено(Номенклатура.ВерсияДанных) Тогда
				
				Если стрк.isFolder = Истина Тогда
					
					СоздатьГруппу(стрк);
					
					мЛоггер.инфо("создана группа= "+гуидСтрока+", "+НаимДляЛога);
				...
				мЛоггер.инфо("не найден Элемент= "+гуидСтрока+", "+НаимДляЛога);
				
				ВСЕГО_ПРОПУЩЕНО = ВСЕГО_ПРОПУЩЕНО + 1;
			...
			ЗаполнитьРеквизиты(НоменклатураОбъект, стрк);
			
			гуидСтрока = стрк.Parent;
			УИД_Родитель = Новый УникальныйИдентификатор(гуидСтрока);

			НоменклатураОбъект.Родитель = Справочники.Номенклатура.ПолучитьСсылку(УИД_Родитель);
			
			
			Если НЕ стрк.isfolder = истина Тогда
				
				НоменклатураОбъект.ВидНоменклатуры = ОпределитьВидНоменклатуры(стрк.VidVosproizv, стрк.TNVED);
				
				Попытка
					НоменклатураОбъект.КодТНВЭД = НайтиСоздатьКодТНВЭД(стрк.TNVED, стрк.TNVED_Kod);
				Исключение       
					т = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				    мЛоггер.ерр("Не удалось создать Классификатор ТН ВЭД! Причина: "+т)
				КонецПопытки;
			...
			ВСЕГО_ОБНОВЛЕНО = ВСЕГО_ОБНОВЛЕНО + 1;
		...
		"Загружено номенклатуры %4 из %3. Пропущено номенклатуры %5", 
...
// ЗАполняет реквизиты объекта
//
// Параметры:
//	СтрокаРекордсета 	- строка ТЗ - 
//
Процедура ЗаполнитьРеквизиты(ОбъектДаных, СтрокаРекордсета)
...
//	Тип: Тип_значения
//
Функция ПолучитьГУИДстрокой(ComSafeArray)
	
	Массив = ComSafeArray.Выгрузить();
	бин = ПолучитьSQLBinaryFromComSafeArray2(Массив);
	ггг = БинариВГуидСтроку(Бин);
...
//
// Параметры:
//	ComSafeArray 	- гуид в виде массива чисел - 
//
// Возвращаемое значение:
//	Тип: Тип_значения
//
Функция ВидВоспроизводства(ComSafeArray)
	...
	Если ВРЕГ(VidVosproizv) = "21DBE6B7-31B7-DF67-4BB0-CD4A7D143950" Тогда
		_VidVosproizv = "Закупка";
	ИначеЕсли ВРЕГ(VidVosproizv) = "C8AF82A7-774F-4530-442D-9281FFD6A9ED" Тогда
		_VidVosproizv = "Производство";
	ИначеЕсли ВРЕГ(VidVosproizv) = "A7199F8A-AB4B-7745-4108-C4B1FE652ABF" Тогда
		_VidVosproizv = "Переработка";
	ИначеЕсли ВРЕГ(VidVosproizv) = "3367CF96-F66C-BA85-4EFF-DD934DCF15E6" Тогда
		_VidVosproizv = "ПринятыеВПереработку";
	КонецЕсли;
				
	
	Возврат _VidVosproizv;
...
//	Параметр1 	- Тип1 - 
//
Процедура СоздатьГруппу(стрк)
	
			гуидСтрока = Сред( стрк.Ref, 2 , 36);
			
			
			УИД = Новый УникальныйИдентификатор(гуидСтрока);

			
	а = Справочники.Номенклатура.СоздатьГруппу();
	ссылка = Справочники.Номенклатура.получитьссылку(УИД);
	а.УстановитьСсылкуНового(ссылка);
	а.Наименование = стрк.description;	
	а.Записать();
...
//	Тип: Тип_значения
//
Функция ОпределитьВидНоменклатуры(VidVosproizv, TNVED)
	
			
	КодТНВЭД = лев(TNVED,4);
	
	
	//ЕНС. Добавил ветку
	Если ЗначениеЗаполнено(VidVosproizv) Тогда
...
Функция НайтиНоменклатуруВКэше(гуидСтрока)
	...
	Если стрк = Неопределено Тогда
		...
		Исключение      
			т = "не смогли создать Новый УникальныйИдентификатор из ГУИДа: "+строка(гуидСтрока);
		    млоггер.ерр(т);
			ВызватьИсключение т;
		КонецПопытки;
	КонецЕсли;
...
Функция ConnectionString(ИмяСервера, Порт, ИмяПользователя, Пароль, ИмяБазыДанных, ДрайверMSSQL) Экспорт
	...
	Если Врег(ДрайверMSSQL) = "OLEDB" Тогда
		...
		Рез = "Provider=MSOLEDBSQL;Server=%1;Database=%2;UID=%3;PWD=%4;Network Library=DBMSSOCN";
	...
	ИначеЕсли Врег(ДрайверMSSQL) = "ODBC.17" Тогда
		Рез = "Driver={ODBC Driver 17 for SQL Server};Server=%1;Database=%2;UID=%3;PWD=%4;Network Library=DBMSSOCN";
	Иначе
		Рез = "Driver={ODBC Driver 17 for SQL Server};Server=%1;Database=%2;UID=%3;PWD=%4;Network Library=DBMSSOCN";
	КонецЕсли; 
...
// создает строку вида 0x80C14CCC6A0BFFF311E6C10D673B12C1 из массива с байтами. Массив получается из ComSafeArray методом Выгрузить()
// параметры:
...
//	Драйвер - строка - значения см. в методе ConnectionString()
//
Процедура ТестПрочитатьПервые10ШКНаСервере(Знач ИдВызова, Сервер,ИмяБазы,
		Пользователь,Пароль,РазмерПакета,Обновлять,МестоЗапуска,Драйвер)  Экспорт
	adoConnection = Новый COMОбъект("ADODB.Connection");
	Порт=1433;
	...
	connString = ConnectionString(Сервер, Порт, Пользователь, Пароль, ИмяБазы, Драйвер);
	adoConnection.Open(connString);
	...
	Если МестоЗапуска = 0 Тогда 
		// ноут XPS
		queryText = "select TOP 10 cast(_FLD35039RRef as uniqueidentifier) as Goods, 
		|cast(_FLD35040RRef  as uniqueidentifier) as Chars, 
		|_FLD35041 as Barcode 
		|from _InfoRg35038";   
		
	ИначеЕсли МестоЗапуска = 1 Тогда
		// УПП
		queryText = "select TOP 10 _Fld15502_RRRef as Goods, 
	...
	//		
	//		НовСтр = ТЗРекордсет.добавить();      
			Штрихкод 		= recordSet.Fields.Item(2).value;
			сообщить(Штрихкод);
			
			Если КэшКодов.Найти(Штрихкод, "ШК") = Неопределено Тогда
				НовСтр = КэшКодов.Добавить();
				новстр.ШК= Штрихкод;
			Иначе
				recordSet.moveNext();
				продолжить;
			...
			ном = recordSet.Fields.Item(0).value;   //
			...
			Номенклатура = НайтиНоменклатуруВКэше(гуид);      
			
			хар = recordSet.Fields.Item(1).value;   //
			...
			харак = НайтиХарактеристикуВКэше(гуид);  			
	//		
...
Процедура Тест01_ОбработатьОдинПакет(Знач ИдВызова) Экспорт
	...
	ТЗРекордсет = ТЗРекордсет();
	НовСтр = ТЗРекордсет.добавить();
	Новстр.barcode = "12345678";
	НовСтр.goods= "bc089036-0a05-11ef-8bfb-04ed33c124eb";
	НовСтр.chars= "bc089038-0a05-11ef-8bfb-04ed33c124eb";
	НовСтр = ТЗРекордсет.добавить();
	Новстр.barcode = "987654321";
	НовСтр.goods= "bc089036-0a05-11ef-8bfb-04ed33c124eb";
	НовСтр.chars= "bc089039-0a05-11ef-8bfb-04ed33c124eb";
	
	ДопПараметры = Новый структура;
	ДопПараметры.Вставить("Обновлять", Истина);
	ОбработатьПакет_Фон(ИдВызова, ТЗРекордсет, 0, ДопПараметры);
...
КэшКодов = Новый ТаблицаЗначений;
КэшКодов.Колонки.Добавить("ШК");//строка 
КэшКодов.Индексы.Добавить("ШК");
