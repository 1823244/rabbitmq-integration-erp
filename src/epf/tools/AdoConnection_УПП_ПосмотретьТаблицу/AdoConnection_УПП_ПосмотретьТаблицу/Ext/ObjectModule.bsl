...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование","AdoConnection_УПП_ПосмотретьТаблицу");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация","AdoConnection_УПП_ПосмотретьТаблицу");
	...
	ДобавитьКоманду(ПараметрыРегистрации.Команды, 
		"Открыть форму : AdoConnection_УПП_ПосмотретьТаблицу",
		"Форма_AdoConnection_УПП_ПосмотретьТаблицу",
		ТипКоманды, 
		Ложь) ;
...
Процедура ЗагрузитьВсеНаСервере(Знач ИдВызова,ИмяСервера,ИмяБазыДанных,
	...
	млоггер.инфо("просмотреть таблицу запуск.");
	...
	while not recordSet.eof do
			...
			млоггер.инфо("ref " +  ПолучитьГУИДстрокой(recordSet.Fields.Item(0).value  ));
			млоггер.инфо("descr" +  recordSet.Fields.Item(1).value  );
	...
	млоггер.инфо("просмотреть таблицу: ЗАВЕРШЕНИЕ");
...
Процедура ОбработатьПакет_Фон(Знач ИдВызова, ТЗРекордсет, НомерЗадания, ДопПараметры) Экспорт	
	...
	//		
			...
			мЛоггер.инфо("стрк.Goods = "+стрк.Goods);
			гуидСтрока = Сред( стрк.Goods , 2 , 36);
			мЛоггер.инфо("гуидСтрока = "+гуидСтрока);
			...
			гуидСтрока = Сред( стрк.Chars , 2 , 36);
...
Функция ТЗрекордсет() Экспорт
	...
	//ТЗРекордсет.Колонки.Добавить("Goods", ТипСтрока);  //строка
	//ТЗРекордсет.Колонки.Добавить("Chars", ТипСтрока);//строка
	ТЗРекордсет.Колонки.Добавить("Goods");  //строка
	ТЗРекордсет.Колонки.Добавить("Chars");//строка
