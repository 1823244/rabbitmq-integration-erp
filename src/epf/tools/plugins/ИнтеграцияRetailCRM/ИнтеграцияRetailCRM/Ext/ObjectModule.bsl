...
Функция ОписаниеПлагина(КонтекстЯдра, ВозможныеТипыПлагинов) Экспорт
	...
	Результат.Вставить("Представление", "Хэлперы для тестирования интеграций с RetailCRM");
...
// Для простого теста плагина - лишь поверить, что плагин выполняется без ошибок.
// Загружает объект.
//
// Параметры:
//	Параметр1 	- Тип1 - 
...
// Возвращаемое значение:
//	Тип: структура, поля:
//		* ДанныеСтруктура - структура - получена из json
...
//	Тип: Тип_значения
//
Функция ПолучитьОбработкуRetailCRM() экспорт
	...
	Запрос.Текст = 
		...
		|	Справочник.ДополнительныеОтчетыИОбработки КАК спр
		|ГДЕ
		|	спр.ИмяОбъекта = ""crm_RetailCRMОбработка""";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	Выборка = РезультатЗапроса.Выбрать();
	Пока выборка.Следующий() Цикл
		ОбъектПлагина 	= ДополнительныеОтчетыИОбработки.ОбъектВнешнейОбработки(Выборка.Ссылка);	
