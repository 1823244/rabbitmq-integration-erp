...
Процедура ЗагрузитьВсеНаСервере(Знач ИдВызова,ИмяСервера,ИмяБазыДанных,
	...
	Если МестоЗапуска = 0 Тогда 
		// ноут XPS
		queryText = "select  top 10
		|_FLD35039RRef as Goods, 
	...
	ИначеЕсли МестоЗапуска = 1 Тогда
		// УПП
		queryText = "select -- TOP 25000
		|_Fld15502_RRRef  as Goods, 
		|_Fld15504RRef   as Chars, 
		|_Fld15501 as Barcode
		//|_fld15500 as typeOfBarCode
		|from _InfoRg15498"; 
		//  where   _fld15500 = 'some guid'
	...
	Иначе 
		...
		while not recordSet.eof do
			...
			while not recordSet.eof И КолВПакете < РазмерПакета do
				...
				НовСтр = ТЗРекордсет.добавить();      
				НовСтр.barcode 		= recordSet.Fields.Item(2).value;
...
Процедура ОбработатьПакет_Фон(Знач ИдВызова, ТЗРекордсет, НомерЗадания, ДопПараметры) Экспорт	
	...
	Попытка
		...
		Для каждого стрк Из ТЗРекордсет Цикл
			...
			// пока не надо
			//
			//TypeOfBarcodeГУИД = ПолучитьГУИДстрокой(стрк.TypeOfBarcode);
			//
			//Если ВРЕГ(TypeOfBarcodeГУИД) = "DDC03064-37B8-4688-9401-07AC8346E621" Тогда
			//	//code128
			//	Штрихкод = Сред(Штрихкод, 2, 13);
			//	
			//КонецЕсли;
			//
			...
			НЗ.Отбор.Штрихкод.Установить(Штрихкод);
			НЗ.Прочитать();
			...
			мЛоггер.дебаг("Штрихкод: %1", Штрихкод);
			
			НоменклатураГУИД = ПолучитьГУИДстрокой(стрк.Goods);
			ХарактеристикаГУИД = ПолучитьГУИДстрокой(стрк.Chars);
			
			НовСтр.Номенклатура = НайтиНоменклатуруВКэше(НоменклатураГУИД);      
			
			НовСтр.Характеристика = НайтиХарактеристикуВКэше(ХарактеристикаГУИД);
...
//
// Параметры:
//	Массив 	- Тип1 - массив чисел - это байты
//
// Возвращаемое значение:
//	Тип: Тип_значения
//
Функция ПолучитьГУИДстрокой(Массив)
...
Функция ТЗрекордсет() Экспорт
	...
	ТЗРекордсет.Колонки.Добавить("Goods");  //comSafeArray
	ТЗРекордсет.Колонки.Добавить("Chars");  //comSafeArray
	ТЗРекордсет.Колонки.Добавить("barcode");
	
	//ТЗРекордсет.Колонки.Добавить("TypeOfBarcode");//comSafeArray
