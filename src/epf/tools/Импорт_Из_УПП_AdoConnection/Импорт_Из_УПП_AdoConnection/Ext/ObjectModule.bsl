...
Процедура ЗагрузитьВсеНаСервере(ИдВызова,ИмяСервера,ИмяБазыДанных,ИмяПользователя,Пароль) Экспорт
	...
	// УПП
	//queryText = "select * from _InfoRg15498";
	// тест, UN
	...
	while not recordSet.eof do
		...
		while not recordSet.eof И КолВПакете < 10000 do
			...
			номЗначение = ПолучитьSQLBinaryFromComSafeArray(ном);
			харЗначение = ПолучитьSQLBinaryFromComSafeArray(хар);
			НовСтр.Номенклатура 	= БинариВГуидСтроку(номЗначение);  
			НовСтр.Характеристика 	= БинариВГуидСтроку(харЗначение);
...
//	Параметр1 	- Тип1 - 
//
Процедура ОбработатьОдинПакет(Знач ИдВызова, НомерЗадания, ТЗРекордсет) Экспорт
	...
	Попытка
		...
		Для каждого стрк Из ТЗРекордсет Цикл
			...
			НЗ.Отбор.Штрихкод.Установить(стрк.Штрихкод);
			НЗ.Прочитать();
			...
			НовСтр.Номенклатура = НайтиНоменклатуруВКэше(стрк.Номенклатура);
			НовСтр.Характеристика = НайтиХарактеристикуВКэше(стрк.Характеристика);
		...
		млоггер.инфо("ОбработатьОдинПакет ЗАВЕРШЕНИЕ . Номер задания = %2, Время = %1 мс. Загружено штрихкодов %3", строка(Время), НомерЗадания, ТЗРекордсет.Количество());
...
Процедура Тест01_ОбработатьОдинПакет(Знач ИдВызова) Экспорт
	...
	ОбработатьОдинПакет(ИдВызова, 0, ТЗРекордсет);
...
Функция ТЗрекордсет() Экспорт
	...
	ТЗРекордсет.Колонки.Добавить("Номенклатура");
	ТЗРекордсет.Колонки.Добавить("Характеристика");
	возврат ТЗРекордсет;
...
Функция БинариВГуидСтроку(Знач Бин)
	...
	Бин = сред(Бин,2);//уберем 0x слева 
