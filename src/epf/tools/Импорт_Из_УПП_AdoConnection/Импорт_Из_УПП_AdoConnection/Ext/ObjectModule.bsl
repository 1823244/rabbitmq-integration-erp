...
Процедура ОбработатьПакет_Фон(Знач ИдВызова, ТЗРекордсет, НомерЗадания, ДопПараметры) Экспорт	
	...
	//		
			...
			//мЛоггер.инфо(стрк.Goods);
			//мЛоггер.инфо(типЗнч(стрк.Goods));
		...
		млоггер.инфо("ОбработатьОдинПакет ЗАВЕРШЕНИЕ . Номер задания = %2, Время = %1 мс. Загружено штрихкодов %4 из %3", 
			строка(Время), НомерЗадания, ТЗРекордсет.Количество(), сч);
