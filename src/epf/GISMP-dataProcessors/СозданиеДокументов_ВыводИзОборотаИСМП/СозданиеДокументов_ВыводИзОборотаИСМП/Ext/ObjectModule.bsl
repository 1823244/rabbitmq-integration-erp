 Перем мОрганизация;
 Перем мПоказыватьИсключенияЯвно;
...
// Точка входа
//	ПоказыватьИсключенияЯвно - булево - если ЛОЖЬ, то фиксируем ошибки только в ЖР (для запуска из регл задания). Истина - интерактивный запуск, нужно показывать ошибки пользователю
Функция СформироватьДокументыИСМП(ДвиженияКМФулфилментСсылка = Неопределено, ПоказыватьИсключенияЯвно = Ложь) Экспорт
	
	мОрганизация = Справочники.Организации.НайтиПоРеквизиту("ИНН", "6152001000");
	мПоказыватьИсключенияЯвно = ПоказыватьИсключенияЯвно;
	...
	Если РезультатЗапроса.Пустой() Тогда
		Если мПоказыватьИсключенияЯвно Тогда
			ВызватьИсключение "Запрос ничего не выбрал! Возможно, требуется добавить документ ДвиженияКМ в регистр статусов";
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьВыводИзОборотаИСМП(Выборка) Экспорт
	...
	//++ Русакова
	НомерЗаказа = ЛЕВ(Выборка.НомерЗаказаИсточник, 5);
	НаборЗаписей = РегистрыСведений.ксп_ИсходныеДанныеЗаказов.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.id.Установить(НомерЗаказа);
	НаборЗаписей.Прочитать();
	Если НаборЗаписей.Количество() > 0 Тогда
		СсылкаНаРеализацию = НаборЗаписей[0].Реализация;
	Иначе
		СсылкаНаРеализацию = Документы.РеализацияТоваровУслуг.ПустаяСсылка();
	КонецЕсли;
	//-- Русакова 
	...
	ОбъектДанных.ДокументОснование = СсылкаНаРеализацию; // Русакова
...
Функция СформироватьВозвратВОборотИСМП(Выборка) Экспорт
	...
	Для счТовары = 0 По ДокОснование.ДвиженияКодовМаркировки.Количество()-1 Цикл
		...
		// ЕНС 2024-07-04 Так не работает (ошибка 01):
		...
		СтрокаТЧ.ВидПервичногоДокумента = Перечисления.ВидыПервичныхДокументовИСМП.ТоварныйЧек;
		СтрокаТЧ.НаименованиеПервичногоДокумента = "";
		СтрокаТЧ.НомерПервичногоДокумента = Выборка.НомерЗаказаИсточник;
		СтрокаТЧ.ДатаПервичногоДокумента = Выборка.ДатаЗаказаИсточник;
		СтрокаТЧ.Оплачен = Истина;
