...
function SupportSendMessage(btnObj) {
    ...
    if (filesCount > 0) {
        if (filesObj[0].size > MAX_SIZE_MB * 1024 * 1024) {
            $.smallBox({
                title: "Ошибка!",
                content: "Размер файла не должен превышать " + MAX_SIZE_MB + " МБ.",
                color: "#d9534f",
...
BX.ready(function () {
    BX.addCustomEvent("onPullEvent-support", function (command, params) {
        ...
        if (command == 'typing' && window.SID == params.TID && window.UID != params.UID && !(window.UID == undefined && params.UID == '') && $("#support-ticket-wrap").length > 0) {
            ...
            $('.chat-info-message').html('Техподдержка печатает...');
            setTimeout(function () {
        ...
        (async () => {
            ...
            try {
                ...
                if(data.ERROR){
                    ...
                    $.smallBox({
                        title: "Спасибо!",
                        content: 'Ваша оценка получена',
                        color: "#659265",
                        icon: "fa fa-check fadeInRight animated",
                        timeout: 4000
                    });
                    $("#crm-rate").html('<h1 style="font-size: 12px;text-align: center;" class="padding-10">Спасибо! Ваша оценка получена</h1>')
                ...
                $.smallBox({
                    title : "Ошибка!",
                    content : 'Не получилось отправить запрос',
                    color : "#C46A69",
                    icon : "fa fa-warning fadeInRight animated",
                    timeout : 10000
                });
